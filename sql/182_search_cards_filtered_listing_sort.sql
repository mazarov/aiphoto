-- Migration 182: align search_cards_filtered order with listing sort (resolve_route_cards)
-- p_sort: 'new' (created_at DESC) | 'popular' (popularity_score DESC, created_at DESC)
-- Fixes catalog-admin FilterableGrid replacing /trends (and other listings) with view_count order.
...
DECLARE
  v_sort text;
BEGIN
  v_sort := lower(trim(COALESCE(p_sort, 'new')));
  ...
  WHERE (
      p_published = 'all'
      OR (p_published = 'yes' AND c.is_published = true)
      OR (p_published = 'no' AND c.is_published = false)
    )
  ...
  ORDER BY
    CASE WHEN v_sort = 'new' THEN NULL::double precision ELSE c.popularity_score END DESC NULLS LAST,
    c.created_at DESC,
    c.id DESC
  LIMIT p_limit
  OFFSET p_offset;

-- Re-runnable camera-orbit enqueue.
-- 212 used CREATE FUNCTION after DROP of the *old* 22-arg signature only.
-- Re-running 212 hits 42723: the 25-arg function already exists.
-- Do not re-run 212. This file drops both signatures, then CREATE OR REPLACE.

ALTER TABLE public.landing_generations
  ADD COLUMN IF NOT EXISTS edit_kind text;

ALTER TABLE public.landing_generations
  ADD COLUMN IF NOT EXISTS scene_root_id uuid;
...
ALTER TABLE public.landing_generations
  DROP CONSTRAINT IF EXISTS landing_generations_scene_root_id_fkey,
  ADD CONSTRAINT landing_generations_scene_root_id_fkey
    FOREIGN KEY (scene_root_id)
    REFERENCES public.landing_generations(id)
    ON DELETE RESTRICT;

ALTER TABLE public.landing_generations
  DROP CONSTRAINT IF EXISTS landing_generations_camera_orbit_complete,
  ADD CONSTRAINT landing_generations_camera_orbit_complete
    CHECK (
...
CREATE INDEX IF NOT EXISTS idx_landing_generations_camera_scene
  ON public.landing_generations(scene_root_id, created_at)
  WHERE edit_kind = 'camera_orbit';

CREATE INDEX IF NOT EXISTS idx_landing_generations_camera_orbit_active
  ON public.landing_generations(scene_root_id)
  WHERE edit_kind = 'camera_orbit'
    AND status IN ('pending', 'processing');

INSERT INTO public.landing_generation_config (key, value, updated_at)
VALUES ('camera_orbit_enabled', 'false', now())
ON CONFLICT (key) DO NOTHING;

COMMENT ON COLUMN public.landing_generations.edit_kind IS
  'local_edit = remix/delta; camera_orbit = viewpoint change from scene_root. NULL = legacy.';
COMMENT ON COLUMN public.landing_generations.scene_root_id IS
  'First non-orbit generation of a photoshoot. Set only on camera_orbit rows.';
COMMENT ON COLUMN public.landing_generations.camera_pose IS
  'Absolute camera vs root: {azimuthDeg, elevationDeg, distanceRel}.';
...
BEGIN
  ...
  END IF;
  IF v_edit_kind IS NOT NULL AND v_edit_kind NOT IN ('local_edit', 'camera_orbit') THEN
  ...
  IF p_parent_generation_id IS NOT NULL THEN
    ...
    END IF;
    IF (v_modality = 'video' OR v_edit_kind = 'camera_orbit')

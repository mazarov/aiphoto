-- Hide Seedream 4.5 from the picker. Worker path stays for in-flight jobs.
-- Do not edit 215/216/218.
        ...
        SELECT jsonb_agg(
          CASE
            WHEN model.value->>'id' = 'seedream-4.5'
              THEN model.value || jsonb_build_object('enabled', false)
            ELSE model.value
          END

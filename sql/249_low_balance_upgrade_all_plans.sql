-- Low-balance grant is −20% on every catalog plan, not start-only.
-- Untargeted 10/20% grants are unique per campaign, so this 20% can
-- coexist with an abandon 10% after we clear target_plan_id.
...
CREATE UNIQUE INDEX landing_pricing_offers_unconsumed_standard
  ON public.landing_pricing_offers(shared_user_id, source_template_id)
...
COMMENT ON FUNCTION public.landing_ensure_low_balance_upgrade(uuid, uuid) IS
  'Idempotent: one-time −20% on every catalog plan, TTL 24 hours, when flag is on, balance <= 15, and the only live payment is the 99 RUB trial/30 pack.';

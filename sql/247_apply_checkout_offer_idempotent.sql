-- Re-applying a live offer onto the same payment must keep the stored
-- amount. Passing the already-discounted figure as catalog would otherwise
-- stack the percent (299 → 239 → 191).
...
COMMENT ON FUNCTION public.landing_apply_checkout_offer(uuid, uuid, text, numeric) IS
  'Lock a live pricing offer onto a YooKassa/Robokassa payment. Re-apply on the same payment returns the stored amount. Output columns are quoted_*.';

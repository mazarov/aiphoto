-- Keep wardrobe from the user's reference photo on initial I2I.
-- Do not edit 230. New optional enqueue arg defaults to replace.

ALTER TABLE public.landing_generations
  ADD COLUMN IF NOT EXISTS wardrobe_policy text NOT NULL DEFAULT 'replace';

ALTER TABLE public.landing_generations
  DROP CONSTRAINT IF EXISTS landing_generations_wardrobe_policy_valid,
  ADD CONSTRAINT landing_generations_wardrobe_policy_valid
    CHECK (wardrobe_policy IN ('replace', 'keep'));

COMMENT ON COLUMN public.landing_generations.wardrobe_policy IS
  'Initial I2I wardrobe contract: replace = dress from prompt (default); keep = lock garments from the input photo.';

ALTER TABLE public.landing_generation_preferences
  ADD COLUMN IF NOT EXISTS preserve_outfit boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.landing_generation_preferences.preserve_outfit IS
  'Last compose chip «Оставить одежду». True = keep wardrobe from the selected photo.';

INSERT INTO public.landing_generation_config (key, value)
VALUES ('preserve_outfit_enabled', 'false')
...
CREATE OR REPLACE FUNCTION public.landing_enqueue_generation(
  ...
  p_wardrobe_policy text DEFAULT 'replace'
)
...
DECLARE
  ...
  v_wardrobe_policy text := COALESCE(NULLIF(btrim(p_wardrobe_policy), ''), 'replace');
...
BEGIN
  ...
  END IF;
  IF v_wardrobe_policy NOT IN ('replace', 'keep') THEN
    RAISE EXCEPTION 'invalid_wardrobe_policy' USING ERRCODE = 'P0001';
  END IF;
  ...
  IF v_wardrobe_policy = 'keep' AND (
    v_modality IS DISTINCT FROM 'image'
    OR v_edit_kind IS NOT NULL
    OR p_parent_generation_id IS NOT NULL
    OR v_vibe_id IS NOT NULL
    OR v_path_count < 1
  ) THEN
    v_wardrobe_policy := 'replace';
  END IF;
...
REVOKE ALL ON FUNCTION public.landing_enqueue_generation(
  uuid, uuid, text, text, uuid, text, text, text, text, int, text[], uuid, text, text, boolean, uuid, text, text, int, uuid, uuid, text, uuid, jsonb, text

-- AI photoshoot: one image job, 2x2 contact sheet, 10 credits.
-- Copies latest landing_enqueue_generation from 213; do not edit 213.
...
COMMENT ON COLUMN public.landing_generations.photoshoot_plan IS
  'Worker vision plan: {theme, shots[4]}. Set after planner, NULL at enqueue.';
COMMENT ON COLUMN public.landing_generations.edit_kind IS
  'local_edit = remix/delta; camera_orbit = viewpoint; photoshoot = 2x2 pose sheet. NULL = legacy.';
...
BEGIN
  ...
  IF v_edit_kind = 'photoshoot' THEN
    ...
    END IF;
    IF p_parent_generation_id IS NULL OR v_edit_instruction IS NULL THEN
  ...
  IF p_parent_generation_id IS NOT NULL THEN
    SELECT requester_auth_user_id, status, result_storage_bucket, result_storage_path, modality, edit_kind
      INTO v_parent_requester, v_parent_status, v_parent_bucket, v_parent_path, v_parent_modality, v_parent_edit_kind
    ...
    END IF;
    IF v_edit_kind = 'photoshoot' AND v_parent_edit_kind = 'photoshoot' THEN

-- Low-balance −20% after any first live purchase, not only the 99 RUB trial pack.
...
COMMENT ON FUNCTION public.landing_ensure_low_balance_upgrade(uuid, uuid) IS
  'Idempotent: one-time −20% on every catalog plan, TTL 24 hours, when flag is on, balance <= 30, and the user has exactly one live payment of any plan.';

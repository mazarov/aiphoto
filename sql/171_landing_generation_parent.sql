-- Iterative image editing: a generation can use the completed result of an
-- owned generation as its sole image input.

ALTER TABLE public.landing_generations
  ADD COLUMN IF NOT EXISTS parent_generation_id uuid;

ALTER TABLE public.landing_generations
  DROP CONSTRAINT IF EXISTS landing_generations_parent_generation_id_fkey,
  ADD CONSTRAINT landing_generations_parent_generation_id_fkey
    FOREIGN KEY (parent_generation_id)
    REFERENCES public.landing_generations(id)
    ON DELETE RESTRICT;

CREATE INDEX IF NOT EXISTS idx_landing_generations_parent_active
  ON public.landing_generations(parent_generation_id)

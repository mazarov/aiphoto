...
bot.on("photo", async (ctx) => {
  ...
  if (!session.state?.startsWith("assistant_") && !["processing"].includes(session.state)) {
  ...
  if (session.generation_type === "avatar_demo" && session.state === "confirm_result") {
...
bot.on("text", async (ctx) => {
  ...
  // === AI Assistant: re-route to assistant after photo generation ===
  // If user was in assistant flow and photo was generated, session.state moved to
  // confirm_result/wait_style/etc. but assistant_session is still active.
  ...
  if (session.state !== "wait_style") {
    if (session.state === "wait_photo") {
      ...
      const msg = lang === "ru"
        ? "Нажми 🤖 Помощник для нового фото, или 🎨 Стили для ручного режима."
        : "Tap 🤖 Assistant for a new photo, or 🎨 Styles for manual mode.";
...
bot.action("cancel", async (ctx) => {
  ...
  const session = await getActiveSession(user.id);
  if (session?.state === "wait_payment") {
    const nextState = "wait_style";
    await supabase
      .from("photo_sessions")
...
bot.on("successful_payment", async (ctx) => {
  ...
  if (finalUser) {
    ...
    const isWaitingForCredits = session?.state === "wait_payment";

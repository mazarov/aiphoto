...
export function FilterableGrid({
  ...
  const allTags = useMemo(() => {
    ...
    for (const c of cards) {
      for (const slug of getSeoTagSlugs(c.seo_tags)) set.add(slug);
    }
    if (filterResults) {
      for (const c of filterResults) {
        for (const slug of getSeoTagSlugs(c.seo_tags)) set.add(slug);
      }
    }
  ...
  const cardIds = useMemo(() => filtered.map((c) => c.id), [filtered]);

  return (
    <CardInteractionsProvider cardIds={cardIds}>
      <div>
        {datasetOnly && filters.dataset && (
          <p className="mb-4 text-sm text-zinc-600">
            Датасет:{" "}
            <span className="font-mono font-medium text-zinc-900">{filters.dataset}</span>
            {filterTotal != null && (
              <span className="text-zinc-400"> — {filterTotal} карточек</span>
            )}
          </p>
        )}

        {(isIdMode && searching) || (isFilterMode && filterSearching && !filterResults) ? (
            ...
            {isFilterMode && filterHasMore && (
              <div className="mt-8 flex justify-center">
                <button
                  ...
                  onClick={() => doFilterSearch(true)}
                  className="rounded-xl border border-zinc-200 bg-white px-5 py-2.5 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-50 disabled:opacity-50"
                >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="f8ada5bf-3100..."
                      className={`w-full rounded-xl border bg-zinc-50 px-3 py-2 text-sm font-mono placeholder:text-zinc-300 transition-colors ${
                        isIdMode
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFilters((f) => ({ ...f, hasWarnings: e.target.value as Filters["hasWarnings"] }))
                    }
                    disabled={isIdMode}
                    className="w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm text-zinc-700 disabled:opacity-40"
                  >
                    <option value="all">Все</option>
                    <option value="yes">С warnings</option>
                    <option value="no">Без warnings</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select
                    value={filters.hasRuPrompt}
                    onChange={(e) =>
                      setFilters((f) => ({ ...f, hasRuPrompt: e.target.value as Filters["hasRuPrompt"] }))
                    }
                    disabled={isIdMode}
                    className="w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm text-zinc-700 disabled:opacity-40"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFilters((f) => ({ ...f, selectedTag: e.target.value }))}
                    disabled={isIdMode}
                    className="w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm text-zinc-700 disabled:opacity-40"
                  >
                ...
                <div>
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setFilters((f) => ({ ...f, hasBefore: f.hasBefore === "yes" ? "all" : "yes" }))
                    }
                    disabled={isIdMode}

...
- subject_pose: CRITICAL — one dense paragraph a director could shoot from. Do NOT substitute generic "straight to camera" / "neutral head" if the photo has torso turn, shoulder asymmetry, or head tilt.
  Do NOT state numeric degrees for body or head rotation — use qualitative wording only (models miscalibrate degrees and under-report strong turns).
  HEAD–TORSO DECOUPLING: The face can look nearly straight into the lens while the chest/hips are turned well away — describe BOTH. Never infer a "slight" or "mild" torso turn just because the eyes meet the camera; if one shoulder reads clearly larger/closer (foreshortening) and the other recedes, the torso is at least a **strong three-quarter** or **near-profile body** with the head **turned back toward the lens** (classic "look-back").
  ANCHORS (viewer + subject directions): (1) Torso+hips vs camera plane — pick ONE label: **square-on** | **slight turn** | **clear three-quarter** | **strong three-quarter / semi-profile body** | **near-profile body** (hips follow torso unless contradicting pixels). (2) Which shoulder is CLOSER to the lens — subject's left/right AND viewer's left/right once each; note foreshortening if obvious. (3) Head vs torso: rotation/twist relative to ribcage (e.g. head turned back over shoulder vs aligned with torso); tilt toward which shoulder or vertical; neck extension/compression. (4) Face vs lens: full / three-quarter / profile (chin–nose line may be off-axis even if eyes hit lens). (5) Chin: raised / neutral / tucked. (6) Shoulder line: level vs uneven; if uneven, say whether depth (closer shoulder) vs wardrobe (e.g. off-shoulder drape) explains it — do not blame clothing alone when depth asymmetry is visible. (7) Gaze vs head. (8) Arms/hands or "hands out of frame". (9) Legs/feet if visible. (10) Weight/lean/hip if visible.
  FORBIDDEN: symmetric "straight-on" when the photo is asymmetric; any phrase like "~N degrees" or "approximately 20°" for pose. If unsure between slight vs strong turn, prefer the **stronger** reading when shoulder scale/depth asymmetry is visible.
  Examples: (studio) "Bust crop; **strong three-quarter body** — subject's left shoulder much closer (viewer's right), right shoulder deep in space; hips follow; **head turned back** so face nearly to lens; chin neutral; uneven shoulders from pose+garment; hands OOF." (bed) "On back, head on pillow tilted left, right arm up with phone for overhead selfie, left hand near face, legs bent under sheets."
...
Inside "prompt" (priority order): (1) POSE/BODY first — director-level geometry: torso+hips vs camera (**square-on / slight / clear three-quarter / strong three-quarter / semi-profile body** — no numeric degrees), which shoulder closer (subject + viewer sides) and foreshortening, **head rotation relative to torso** (e.g. look-back toward lens), head tilt, chin, face vs lens (full/¾/profile even if eyes hit lens), shoulders, weight/lean, spine twist, arms/hands or "hands OOF", legs if visible. Do not use "slight" body turn when one shoulder clearly dominates the frame. No generic "straight-on" if the photo is asymmetric. (2) Vibe (genre, energy, viewer relationship). (3) Expression tied to pose. (4) Camera, concrete lighting, environment, clothing, grade — rephrase reference biometrics as transferable ("subject's natural eye color"). Long hair/makeup prose lives ONLY in grooming.*.

...
## 9. Транзакции в state="created" висят вечно
Если пользователь выбрал тариф, но не оплатил и не выбрал другой — транзакция остаётся в `created` навсегда.
Не критично: напоминание отправляется один раз (`reminder_sent`), но засоряет базу.
Возможное решение: автоматически менять state на `expired` через 7 дней.
Компонент: `index.ts`, таблица `transactions`

...
| Параметр | Значение |
|---|---|
| Промпт по умолчанию | Gemini 2.5 Flash придумывает короткий motion-сценарий по фото (без описания внешности); fallback `Оживи изображение`. Worker: одно фото = кадр для оживления (`[# Sources @Image1]`). `image_to_video` не принимает второе изображение |

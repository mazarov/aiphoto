...
Эти строки попадают в поле **`prompt`** при создании генерации. Они **не** содержат обёртку для image-gen — в **`generate-process`** вызывается **`assembleVibeFinalPrompt`**: сначала это тело сцены, затем блок **CRITICAL RULES** (`vibe-gemini-instructions.ts`).
...
**«Единый промпт»** в смысле пайплайна — **одна** строка сцены в БД + в `generate-process` **`assembleVibeFinalPrompt`** добавляет **CRITICAL RULES** (роли A/B — отдельные части multimodal до текста). В БД у задачи генерации хранится одно поле **`prompt_text`** — тело сцены без этой обёртки.

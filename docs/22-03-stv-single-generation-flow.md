...
> **Версия документа:** 2026-03-22 (extract `camera`: структурированный ракурс в `LEGACY_EXTRACT_PROMPT_2C23CE94`; §11.5; §5.4.1 merge)  
...
**Extract** всегда использует контракт `2c23ce94`: **8 строковых полей**, инструкция **`LEGACY_EXTRACT_PROMPT_2C23CE94`** (в т.ч. структурированное описание ракурса в **`camera`**: фокалка, дистанция/кадр, высота камеры, горизонтальный угол, roll, DOF), insert с **`prompt_chain = legacy_2c23`**. Ветки modern / one-shot / `EXTRACT_STYLE_INSTRUCTION` **удалены** (см. `landing/src/app/api/vibe/extract/route.ts`, `vibe-legacy-prompt-chain.ts`). Провайдеры Gemini/OpenAI и выбор модели — по-прежнему через `photo_app_config`.

...
## Альтернативы (если API недоступен)

Если `gemini-3-pro-vision` недоступен или дорогой:

1. **Программное наложение текста** — Sharp/Canvas после генерации
2. **DALL-E 3** — хорошо рендерит текст, но другой API
3. **Imagen 3** — Google's image model с улучшенным текстом

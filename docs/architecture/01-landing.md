...
- **Robokassa UX:** официальный `robokassa_iframe.js` загружается один раз по требованию; `Robokassa.Render` получает подписанный сервером payload и `Settings={PaymentMethods:['BankCard','SBP'],Mode:'modal'}`. Платёжный iframe показывается поверх PromptShot, `window.location.assign` не вызывается. `SuccessURL`/`FailURL` ведут на компактные `/payment/robokassa/*` внутри iframe.

...
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия user-facing:** лист 2×2 не отдаём клиенту. `GET /api/generations` и `/:id` ставят `resultUrl` = первый тайл, `photoshootTileUrls` = 4 кадра (`resolvePhotoshootUserFacingResult`). Оверлей без «Скачать лист» и без CSS crop. История и каталог — сетка 4 кадров. UGC publish — **одна** `prompt_cards` + 4 `prompt_card_media` (тайлы, не лист, не `card_split`) + 4 analyzed `prompt_variants`. Старый черновик с одним кадром дописывается на ensure/publish. save-to-library — тайл 1. Split fail валит job.
>
...
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия split:** после I2I worker режет лист `sharp.extract` 2×2 → 4 JPEG sidecar (`lease-N.jpg`), пути в `photoshoot_tile_paths` (SQL `225`). Poll отдаёт `photoshootTileUrls`. Спека `docs/28-08-ai-photoshoot-split.md`.
>

...
| `/api/generation-config` | Конфиг генерации; `modality=image` (default) или `video`. Image: модели из `models` (Nano Banana / PRO / 2 / Lite + **Grok Imagine** `grok-imagine-image-2.0` + **Seedream 4.5 / 5.0 Pro** + **Flux 2 Flex**, OpenRouter-модели по 10 кредитов), `cameraOrbitEnabled` из `camera_orbit_enabled` **или** allowlist, `cameraOrbitModel` из `camera_orbit_model` (дефолт Grok, id должен быть в `models`). Video: модели из `video_models` (Grok 1.5 + Veo Omni Flash + Veo 3.1 Lite), `defaults.model` = `DEFAULT_VIDEO_MODEL` (`veo-3.1-lite-generate-preview`) через `resolveVideoModelId`, 9:16/16:9, 4/6/8/10 сек (Lite без 10), 720p, `enabled` из `video_animate_enabled` **или** allowlist (`azarov.maxim@gmail.com`). SSOT `lib/generation/image-options.ts` |

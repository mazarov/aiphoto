...
- **Robokassa UX:** официальный `robokassa_iframe.js` загружается один раз по требованию; `Robokassa.Render` получает подписанный сервером payload и `Settings={Mode:'modal'}`. `PaymentMethods` намеренно не ограничен: iframe показывает все способы, доступные магазину Robokassa. Backend передаёт `Email` из подтверждённой auth-сессии, поэтому адрес для чека уже заполнен; невалидный/отсутствующий email не отправляется. Платёжная форма открывается поверх PromptShot, `window.location.assign` не вызывается. `SuccessURL`/`FailURL` — `/payment/robokassa/*`: внутри iframe страница закрывает overlay (`ClosePaymentForm` + remove `#robokassa_iframe`) и оставляет исходный экран; если Robokassa редиректит top window — `location.replace` на сохранённый return path. «Вернуться в магазин» не должно оставлять пользователя на `/payment/*`.

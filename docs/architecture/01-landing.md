...
> Последнее обновление: 2026-08-10 (**OAuth return path:** `AuthModal` / `signInWithOAuthProvider` редиректит на `/auth/callback?next=<текущий path>`; callback и client fallback возвращают на страницу старта логина, а не на `/`. Return path дублируется в sessionStorage + cookie `ps_auth_next`.)
>
> Предыдущее обновление: 2026-08-10 (**pricing legal footer:** на max-lg блок оферты/реквизитов без акцента (`mt-auto` в колонке с `min-h` = высота над tab bar). Если контент влезает — прибит к низу; если нет — уезжает вместе с тарифами под `MobileTabBar` (не `position:fixed`).)
>
> Предыдущее обновление: 2026-08-10 (**pricing mobile layout:** `/pricing` на max-lg больше не фиксирует высоту через `100dvh`/`overflow-hidden` внутри listing shell — контент скроллится в `listing-scroll-root` над `MobileTabBar`. Сетка пакетов: `gap-y-4` + `overflow-visible`, чтобы бейдж «Популярный» не наезжал на карточку сверху.)
>
...
- **Mobile UI:** страница живёт в общем listing shell (`Header` + `listing-scroll-root` + `MobileTabBar`). Main на max-lg имеет `min-h` = высота над tab bar; legal footer (`mt-auto`, без border/blur) при достаточном месте сидит внизу, иначе скроллится вместе с карточками. Карточки — 2×2 до `xl`, с увеличенным `gap-y` под absolute-бейдж «Популярный».

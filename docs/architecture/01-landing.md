...
- **Точка входа:** кнопка **Steal the vibe** на `/p/[slug]` (`GenerateButton`, стиль как primary в панели STV), при `NEXT_PUBLIC_ENABLE_TRY_THIS_LOOK` или `debugOpen`.
- **UI:** `GenerationModal` — iframe на **`/embed/stv?...`**, внутри тот же пайплайн, что в расширении (extract → expand → generate, см. § Vibe Pipeline ниже). Ранний отдельный React-флоу (`GenerationModal` + `PhotoUploader` + прямой `POST /api/generate` без vibe) **снят**.

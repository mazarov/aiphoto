...
> Последнее обновление: 2026-08-31 (**photoshoot timing logs:** worker пишет один сводный `photoshoot_timing`: `queueWaitMs`, `inputDownloadMs`, `plannerMs`, `planPersistMs`, `provider`/`providerMs`, `encodeMs`, `sheetUploadMs`, `splitMs`, `tileUploadMs`, `workerMs`, `enqueueToDoneMs`. Шаги: `photoshoot_planner_ok` (`shrinkMs`/`requestMs`), `gemini_request_completed` / `grok_image_request_completed` / `seedream_completed`, `photoshoot_split_ok`. `generation_started` / `generation_completed` — `createdAt` + `queueWaitMs`. Grep `photoshoot_timing`.
>
> Последнее обновление: 2026-08-31 (**хаб ИИ-фотосессии → `/ii-fotosessiya`:** коммерческий кластер «ии фотосессия / по фото». Title `ИИ фотосессия по фото онлайн`, H1 `ИИ фотосессия по своему фото`. L2 `/ii-fotosessiya/{slug}`. 301 `/promty-dlya-ii-fotosessii` и `/:slug` + `/promty-dlya-fotosessii` → новый хаб. Спека `docs/31-08-ii-fotosessiya.md`.
>
...
- **Photoshoot latency:** для `edit_kind=photoshoot` worker пишет сводный JSON `photoshoot_timing` (очередь + download + planner + I2I + encode + upload листа + split/тайлы + `enqueueToDoneMs` от `created_at`). Planner: `shrinkMs` / `requestMs`. I2I: `gemini_request_completed` / `grok_image_request_completed` / `seedream_completed` с `durationMs`. SSOT `web-generation-worker/src/photoshoot-timing.ts`.
...
- **Хаб ИИ-фотосессии (`/ii-fotosessiya`):** отдельный тип, не `[...slug]`. Title «ИИ фотосессия по фото онлайн», H1 «ИИ фотосессия по своему фото». Карусель — карточки-фотосессии. `#primery` — обычные луки без фильтра фотосессии. Порядок: темы → HowTo → примеры → тарифы → FAQ. 17 L2. JSON-LD CollectionPage + BreadcrumbList + HowTo + FAQPage + ItemList. 301 со старого `/promty-dlya-ii-fotosessii`.

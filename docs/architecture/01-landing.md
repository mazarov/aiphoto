...
> Последнее обновление: 2026-03-22 (extension **extractTemperature** в body extract; **`assembleVibeFinalPrompt`**: сцена + **CRITICAL RULES** вместо CRITICAL + Rules (short))
...
- **Extract:** `POST /api/vibe/extract` — body: **`imageUrl`** + опционально **`extractTemperature`** (`number` **0–2** или **null**; иначе **400**; **omit/null** = температура не передаётся в Gemini/OpenAI, дефолт провайдера). SSRF-guard, скачивание изображения, vision → JSON с **ровно 8 полями** по **`LEGACY_EXTRACT_PROMPT_2C23CE94`** (поле **`camera`**: класс фокалки, масштаб кадра, высота относительно глаз, горизонтальный угол, roll, DOF; при двусмысленности — «uncertain»). Extension: пресеты в секции **«Настройки»**, persist в **`stv_state_v2.extractTemperature`**. Провайдеры: **`vibe_extract_llm`**, `vibe_extract_model` / OpenAI (`sql/150_*.sql`). Insert в **`vibes`**: **`style`**, **`prompt_chain` = `legacy_2c23`**. Ответ: **`legacyPromptChain: true`**.

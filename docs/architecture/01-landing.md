...
| `/api/me` | Текущий пользователь + credits + живой грант `{ offer: { percent, expiresAt } \| null }` + `publishRewardRemainingToday`. Гость — полная цена, `offer: null`. Шапка читает баланс; `PricingCards` рисует зачёркнутый каталог |

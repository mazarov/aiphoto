...
| `/api/generations` | Auth-список строк `landing_generations` текущего shared DB user для `/generations`; `limit`≤50, `offset`, peek `limit+1` → `{ generations, hasMore, nextOffset }`; batch lookup связанных `prompt_cards` → `cardId/cardSlug/isPublished`; private no-store. **DELETE** `{ ids: uuid[] }` (≤50) — bulk hard-delete owned rows; result object удаляется только если не используется `prompt_card_media` |

...
/promty-dlya-ii-fotosessii → Хаб «промты для ии фотосессии в нейросетях» (серия луков, не один кадр и не «сделать фото ИИ»). Каркас: hero + карусель фотосессий + коллажи L2 + HowTo + галерея + тарифы `#tarify` + FAQ. Без tools / отзывов / моделей / финального CTA «Собрать кадры». Карусель: `fetchPublishedPhotoshootListingCards` (`photoshoot-listing.ts`). Copy SSOT `promty-dlya-ii-fotosessii-seo-copy.ts`, роуты `promty-dlya-ii-fotosessii-cluster.ts`, коллажи `promty-dlya-ii-fotosessii-page-data.ts`. L2 `/[audience]` — тот же каркас, тексты через `buildFotosessiiChildCopy` (Title/H1 = ключ ребёнка, без «в нейросетях»). Карусель L2: фотосессии ленты ребёнка, иначе луки с фото. Галерея — `resolve_route_cards` тега ребёнка. Sitemap хаб 0.9, дети от 8 карточек 0.85. 301 `/promty-dlya-fotosessii` → хаб. Спека `docs/28-08-promty-dlya-ii-fotosessii.md`. L2: `/muzhskie` (`muzhchina`), `/zhenskie` (`devushka`), `/pary` (`para`), `/den-rozhdeniya` (`den_rozhdeniya`), `/detskie`, `/semeynye` (`semya`), `/studiynye` (`studiynoe`), `/zimnyaya` (`zima`), `/beremennye` (`beremennaya`), `/s-voennymi` (`v_forme`), `/dlya-dvoih` (`vlyublennykh`), `/novogodnyaya` (`novyy_god`), `/vesennie` (`vesna`), `/delovoy-stil` (`delovoe`), `/nyuborn` (`malysh`), `/s-mashinoy`, `/cherno-belye` (`cherno_beloe`).
...
- **Главная (`/`):** `generateMetadata` → `HOMEPAGE_SEO.title` / `description`; canonical; H1 + intro-subtitle из copy-модуля + карусель 50 новых; destinations вместо стартера (в т.ч. хаб `/promty-dlya-ii-fotosessii`); **`#katalog`** = карусель коллекций (`HOMEPAGE_SEO.examples*`); **`HomepageExamplesExplorer`** (`#primery`, H2 «Идеи промтов для фото», без intro); HowTo и FAQ (`HomeIntroAndHowTo` / `HomeFaq`) в каркасе шаблона; JSON-LD **`CollectionPage`** (`isPartOf: WebSite`, `hasPart[].name` = «Промты для фото {label}») + **`FAQPage`** + **`ItemList`** 16 карточек; FAQ — хвосты Wordcraft (что такое / пример / генерация / ссылка на хаб ИИ-фотосессии / лучшие / какая нейросеть), ссылки на `#primery`, `#katalog` и `/promty-dlya-ii-fotosessii`
- **Хаб ИИ-фотосессии (`/promty-dlya-ii-fotosessii`):** отдельный тип, не `[...slug]`. Title/H1 «Промты для ИИ фотосессии в нейросетях». Карусель — карточки-фотосессии. `#primery` — обычные луки без фильтра фотосессии. Порядок: темы → HowTo → примеры → тарифы → FAQ. 17 L2 (аудитория + событие + стиль + объект), в том числе `/cherno-belye`. JSON-LD CollectionPage + BreadcrumbList + HowTo + FAQPage + ItemList. L1 `devushka` больше не держит «ИИ-фотосессии» в H1.

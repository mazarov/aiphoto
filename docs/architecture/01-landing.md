...
| MobileTabBar | `components/MobileTabBar.tsx` | 5-вкладочный таб-бар (max-lg): Каталог / Фото→промт / Сгенерировать (center pill, LexyGPT) / Поиск / Профиль. Регистрирует `registerMobileSearchOpen`, рендерит `ListingMobileSearchSheet` и `MobileProfileSheet`. |
...
| RPC | Назначение |
|-----|-----------|
| `resolve_route_cards` | Карточки по тегам (листинг + меню); **`p_sort`**: `popular` (default) \| `new`. `popular` — query-time popularity score (мигр. `163`) |
| ~~`recalculate_popularity_scores`~~ | **Наследие** (мигр. 160): hourly batch для `views_7d`/`popularity_score`. Не используется после `163` |
...
**`popularity_score`** (миграция `163`, **считается на лету в `resolve_route_cards`**, без cron): `(view_count + react_weight·(likes_count − dislikes_count)) / (1 + age_days/half_life_days)^decay_exponent`, где `age_days` от `created_at`; `react_weight`/`half_life_days`/`decay_exponent` — **`photo_app_config`** (`listing_popularity_react_weight`=3.0, `listing_popularity_half_life_days`=30, `listing_popularity_decay_exponent`=1.0). База — lifetime `view_count` (realtime), лайки — лёгкий бонус, возраст — мягкое затухание. UI показывает **`view_count`** (lifetime). **Наследие (158–160):** колонка `prompt_cards.popularity_score`, `views_7d`, таблица `prompt_card_view_events`, job `recalculate_popularity_scores()` и `src/standalone/recalculate-popularity-scores-standalone.mjs` — в ранжировании **больше не участвуют** (cron на DO не был запущен → score завис в 0 → `popular`==`new`; миграция `163` сняла зависимость от cron). Follow-up: чистка наследия.

**Блоки категорий на главной (`get_homepage_sections`, миграция `164`):** топ-5 карточек на тег сортируются по **той же query-time popularity-формуле**, что листинг (мигр. `163`) → обложка блока = карточка **#1** листинга этой категории. **Кросс-категорийный дедуп обложек убран** (`buildCategorySectionBlocks` берёт `cards[0]`/`cards[1]` напрямую; `pickDeduplicatedPhotos` удалён) — раньше обложка могла быть 2-й/3-й карточкой, если #1 уже показали в более раннем блоке. Передняя карточка = `cards[0]` (#1 popular), задняя декоративная = `cards[1]`.

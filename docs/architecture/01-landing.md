...
> Последнее обновление: 2026-09-01 (**video animate-scenario = выбранный кадр:** после плитки «Видео» ручка `POST /api/generate/animate-scenario` больше не зовётся только в `enterVideoCompose`. SSOT `resolveVideoAnimateScenarioSource` + эффект: parent XOR одно library-фото. Видео — radio/cap 1, как «Промт по фото». Без кадра плитка открывает «Ваши фото». Catalog Repeat с не-generic `Motion:` ручку не зовёт. Спека `docs/01-09-video-animate-scenario.md`.
>
...
| `/api/generate/animate-scenario` | POST (auth): когда video-compose получил кадр — плитка «Видео» + выбранное фото, загрузка в video-режиме, или «Оживить» на готовом фото. `gemini-2.5-flash` смотрит owned image (parent generation XOR upload path), возвращает короткий RU-сценарий (1–2 предложения) **с этого кадра** (frame 0, без lead-in). Клиентский SSOT: `resolveVideoAnimateScenarioSource` / `shouldRequestVideoAnimateScenario` в `lib/compose-modality-prompt.ts` (эффект в `CardInlineGeneratePanel`, не разовый вызов на клике плитки). Видео — одно фото. Catalog Repeat с не-generic `Motion:` ручку не вызывает. Исходный image-промпт не подмешивается, stash-ится и возвращается в фото-режиме. Не списывает кредиты. Текст сценария: `lib/video-animate-scenario.ts`. Proxy как у remix. |
...
| `/api/payments/yookassa/unpaid-banner` | GET (auth): latest YooKassa row за 24 ч + live offer. Без reconcile. Плашка `UnpaidCheckoutBanner`. |
...
- **Return UX:** клиент polling с backoff ~2→5→10 с до ~20 попыток (~2–3 мин) если есть `?payment=`. Иначе `YooKassaReturnStatus` один раз (и на `visibilitychange`) бьёт `POST /api/payments/yookassa/open-reconcile`. Сервер reconcile’ит и локальный `canceled`, если ещё нет `credited_at`. Не вешать сверку на `GET /api/me`. Если latest create всё ещё без `credited_at` — `UnpaidCheckoutBanner` над навбаром 24 ч (`GET /api/payments/yookassa/unpaid-banner`). Пока в URL есть `?payment=`, бар скрыт. С 15-й минуты и живом гранте 25% — copy про 60 мин + countdown до `expires_at` письма. «Продолжить» делает новый YooKassa create и уходит на hosted URL. Крестик пишет `payment_id` в localStorage.

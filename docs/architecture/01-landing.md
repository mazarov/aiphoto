...
| `/api/generations/[id]` | GET: статус/результат генерации. **DELETE**: hard-delete owned row; result object сохраняется при наличии ссылки из `prompt_card_media` (UGC `prompt_cards` не удаляется) |

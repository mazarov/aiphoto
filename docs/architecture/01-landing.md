...
> Последнее обновление: 2026-08-01 (**client_source на генерациях:** `landing_generations.client_source` — атрибуция клиента `site | embed_stv | extension_stv | extension_lite | unknown`; резолвер `landing/src/lib/client-source.ts` (`X-Client` → Origin → Host); запись в `POST /api/generate`; STV шлёт `X-Client` (`embed_stv` / `extension_stv`); CORS Allow-Headers включает `X-Client`; миграция `sql/168_landing_generations_client_source.sql`. Предыдущее: **feature/internal-generation-repeat:** на `/p/[slug]` и в модалке листинга (`CardPageClient`) CTA «Повторить» открывает **inline compose** (`CardInlineGeneratePanel`) вместо правого STV drawer: upload своего фото + model/aspect/size → прямой `POST /api/generate` с промптом карточки (`vibeId=null`). Временный `STV_OPEN_GENERATE_DEBUG` — upload/generate/poll без auth и без списания кредитов (owner = guest DB row). Listing overlays (`PromptCard`/`GroupedCard`) и `/embed/stv` без изменений в этом шаге; `GenerationModal` остаётся в layout. Предыдущее: STV web LexyGPT-style canvas + `STV_GUEST_MODE`.)
...
- **Floating CTA «Add to Chrome»** на **`/foto-v-promt`**: **`reachGoal('foto_v_promt_add_to_chrome_click')`** (`FotoVPromtFloatingCta` → Chrome Web Store, `getAiImageDescriberChromeUrl('foto_v_promt_floating_cta')`). UTM для GA4 листинга CWS: `utm_source=promptshot.ru`, `utm_medium=cpc`, `utm_campaign=foto_v_promt`, `utm_content` по placement (`foto_v_promt_floating_cta` \| `foto_v_promt_remix_hint` \| `foto_v_promt_json_ld`).
...
- **Точка входа карточки `/p`:** `CardPageClient` передаёт в `LexyGptGenerateButton` `onInternalGenerate` → `setInlineGenerateOpen(true)`. Desktop aside скрывает prompt/tags/reactions/Copy и показывает **`CardInlineGeneratePanel`**; mobile — нижний sheet поверх immersive chrome. Back закрывает compose.
- **Inline engine:** `CardInlineGeneratePanel` — upload (`POST /api/upload-generation-photo` + `prepareUploadFile`), опции из `GET /api/generation-config`, затем `POST /api/generate` с текстом промпта карточки, `vibeId=null`, `cardId` для attribution; poll `GET /api/generations/:id` до completed/failed. Без extract/expand и без iframe `/embed/stv`.
- **Listing overlays:** `PromptCard` / `GroupedCard` без `onInternalGenerate` — прежний маршрут (STV drawer / LexyGPT по наличию `cardId`). CTA `/foto-v-promt` без `cardId` → LexyGPT.
- **STV drawer (legacy / listing):** `GenerationContext.openGenerationModal` → **`GenerationModal`** (iframe **`/embed/stv`**). Web composer: непустой `webPromptDraft` → прямой generate (`vibeId=null`); пустой draft → vibe `extract → expand → generate`. Chrome extension — три вкладки без изменений.

#### Временный open-generate debug (карточка)

- **Флаг:** `STV_OPEN_GENERATE_DEBUG=1`; если не задан — включён только при `NODE_ENV=development`. `0`/`false` выключают bypass.
- **Эффект:** `POST /api/upload-generation-photo`, `POST /api/generate`, `GET /api/generations/:id` не требуют session; generate с `creditsCharged=0` (без `landing_deduct_credits`); DB owner — `resolveGuestOwnerDbUserId` / `STV_GUEST_OWNER_USER_ID` (нужна хотя бы одна строка в `landing_users` из‑за FK на auth).
- **Назначение:** временный обход для отладки inline compose; в проде держать выключенным, пока нет нормального auth/credits flow.
...
- **Кредиты / guest identity:** `/api/me` показывает гостю виртуальный баланс `999`; `/api/generate` при `user.is_anonymous=true` ставит `credits_spent=0` и пропускает `landing_deduct_credits`. **Identity split (Dockhost):** GoTrue `admin.createUser`/`getUserById` могут успеть, а Postgres FK `landing_users.id → auth.users` всё равно reject. Guest mode поэтому берёт **уже существующий** `landing_users.id` (кэш `photo_app_config.stv_guest_owner_user_id` / `STV_GUEST_OWNER_USER_ID` / oldest row) как `landing_generations.user_id`. Нужен хотя бы один OAuth-профиль на сайте. Storage paths остаются под anonymous JWT `user.id`.
...
- **Атрибуция клиента (`client_source`):** при create пишется нормализованный источник (`site` | `embed_stv` | `extension_stv` | `extension_lite` | `unknown`) через `resolveClientSource` (`landing/src/lib/client-source.ts`). Приоритет: заголовок **`X-Client`** (allowlist) → `Origin` (chrome-extension / promptshot hosts) → `Host` (same-origin site). STV: `boot-web.js` → `embed_stv`, `boot-chrome.js` → `extension_stv`; `stv-core.js` `api()` инжектит `X-Client`. Inline compose (`CardInlineGeneratePanel`) без заголовка резолвится в `site` по Host. CORS: `Access-Control-Allow-Headers` включает `X-Client`. Миграция: `sql/168_landing_generations_client_source.sql`.
...
- Карточный маршрут (`/p/[slug]`, модалка листинга) использует **inline compose** (`CardInlineGeneratePanel`), а не `GenerationModal`. Не зависит от `NEXT_PUBLIC_ENABLE_TRY_THIS_LOOK`.
...
| CardModal | `components/CardModal.tsx` | Overlay карточки; desktop — transparent wide shell (`max-w-7xl`), тёмный close; mobile — white card / immersive |
...
| `landing_generations` | История web-генераций (`vibe_id`, `client_source`: site / embed_stv / extension_stv / extension_lite / unknown) |
...
| RPC | Назначение |
|-----|-----------|
| `resolve_route_cards` | Карточки по тегам (листинг + меню); **`p_sort`**: `popular` (default) \| `new`. `popular` — **materialized** `prompt_cards.popularity_score` (мигр. `166`, индекс `158`) |
...
**Сортировка листингов категорий (`/[...slug]/`, миграции `158–161`):** UI — переключатель **`ListingSortToggle`** («Популярное» \| «Новое»), выбор в **`sessionStorage`** `promptshot_listing_sort` + опционально **`?sort=new`** в URL. SSR и API читают **`sort`**. Страница **`/new`** всегда `sort=new` (`fixedSort`), без переключателя и без sessionStorage-sync (`useListingSort({ disabled: true })`).

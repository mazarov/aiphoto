...
- **Video Interactions payload (Omni Flash):** SSOT `web-generation-worker/src/video-interaction.ts` → `buildVideoInteractionRequest`. Официальный unary: `background=false`, `store=false`, `stream=false`. `video_config` только `{ task: "image_to_video" }`; `aspect_ratio` в `response_format`. Текст — `assembleVideoMotionPrompt`: `[# Sources @Image1]` starting frame; если есть аплоад родителя — второй part Image2 + `[# References @Image2]` (identity only). Submit логирует `httpStatus`, `errorCode`, `imageBytes`, `hasIdentityReference`, `identityPath`, `identityBytes`, `linkedGenerationId`. Источник кадра: `video_input_resolved`. `user_photos` = аплоад; если это копия из «Использовать» (`generation-<uuid>.jpg` / `landing_user_photos.source_generation_id`, SQL **199**) — worker поднимает generation result как Image1 и исходный аплоад как Image2. Иначе Image2 = тот же аплоад (identity reference). `generation_result` = готовое фото + Image2 из `input_photo_paths` родителя (walk, max 8). `background=true` на consumer Gemini даёт opaque `invalid_request`.

...
>
> Последнее обновление: 2026-08-16 (**listing pagination reliability:** SSR отдаёт 10 ranked rows, затем `InfiniteGrid` запрашивает порции по 24 (`offset`: 10, 34, 58…). SSOT `useListingSentinelLoadMore` сохраняет `IntersectionObserver` и bounded drain после append, а passive scroll + rAF восстанавливает подгрузку, если быстрый жест полностью перескочил 1px-sentinel. Scroll anchoring не запускает бесконечный drain. Временный HTTP/network failure не превращается в `hasMore=false`: offset сохраняется, показывается явный retry. Каталог и `/search` делят один хук.)
>

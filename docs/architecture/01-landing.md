...
> Последнее обновление: 2026-08-27 (**listing query vectors:** 4 birthday SSOT `q` хранят Gemini query embed в `listing_query_embeddings` (SQL `223`). Listing читает вектор до Gemini; miss → live 8000 мс + upsert. Cron `visual-embeddings` греет только эти 4. `/api/search` по-прежнему live 800 мс, в таблицу не пишет.
>
...
| `/api/cron/visual-embeddings` | POST, `Authorization: Bearer $CRON_SECRET`: enqueue missing canonical-photo embeddings + claim/lease Gemini Embedding 2 batch + warm 4 birthday listing query vectors (`listing_query_embeddings`) |
...
**Кластер «день рождения»:** хаб остаётся `/sobytiya/den-rozhdeniya` (не переезжает на `/den-rozhdeniya`). Живые дети — только L2 `/sobytiya/den-rozhdeniya/{alias}` (`devushki`, `deti`, `muzhchiny` и object-сегменты). L3 audience×object убит: 301 на audience L2 (`DEN_ROZHDENIYA_PERMANENT_REDIRECTS` + `birthdayRetiredL3RedirectPath` в `[...slug]`). Старые audience-first L2/L3 тоже на L2, не на L3. Search-backed sitemap — только слабые L2 (`muzhchiny`, `s-detskim-foto`, `s-shampanskim`, `so-lvom`). Хаб в sitemap как L1-тег, девушке / детям / с тортом из combo-кэша. Без L3. Каноникал occasion-first; pathname ≠ `canonicalPath` → 301. Ручной SEO L2 — combo-ключи `devushka+den_rozhdeniya` и т.п. в `seo-content.ts`. Видимый роутер под H1 — `ListingClusterChipGroup` из SSOT. Хаб и L2 девушке / детям / с тортом — `resolve_route_cards` (сорт по новизне). L2 мужчине / с детским фото / с шампанским / со львом — свои поисковые запросы без tag-фильтра. Gemini budget actor `system` только на хабе. Чужой `/api/listing?q=` — FTS без эмбеддингов и без birthday-фильтров. `/api/search` остаётся на text window 100. Поле поиска в explorer — `/api/search`. Кластерные URL включают общий `Footer` вместе с generate dock. **Антиканнибализация copy:** хаб держит только «промты для фото на день рождения»; L2 не повторяют чужие модификаторы; do-интент — только `/generaciya-foto/na-den-rozhdeniya`.
...
| `listing_query_embeddings` | 768-d Gemini **query** vectors только для 4 birthday listing SSOT `q` (SQL `223`). Не общий кэш `/api/search` |

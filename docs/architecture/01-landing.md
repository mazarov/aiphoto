...
> Последнее обновление: 2026-03-22 (**extract:** `extractTemperature`; **`composition`** в `LEGACY_EXTRACT_PROMPT_2C23CE94` — структура под passthrough expand; **`assembleVibeFinalPrompt`**: сцена + **CRITICAL RULES**)
...
- **Extract:** `POST /api/vibe/extract` — body: **`imageUrl`** + опционально **`extractTemperature`** (`number` **0–2** или **null**; иначе **400**; **omit/null** = температура не передаётся в провайдер). SSRF-guard, скачивание изображения, vision → JSON **8 полей** по **`LEGACY_EXTRACT_PROMPT_2C23CE94`**: **`camera`** — чеклист ракурса/оптики; **`composition`** — абзац под **passthrough** в expand (**`buildLegacyVibeFullPromptBody`** без LLM, текст под меткой **Composition** дословно): плейсмент, кроп, вертикальный баланс в кадре, foreground/midground/background, линии, негативное пространство; **без** дублирования **`camera`**. Extension: пресеты в **«Настройках»**, **`stv_state_v2.extractTemperature`**. Провайдеры: **`vibe_extract_llm`**, `vibe_extract_model` / OpenAI (`sql/150_*.sql`). Insert **`vibes.style`**, **`prompt_chain` = `legacy_2c23`**, **`legacyPromptChain: true`**.
...
- **generate-process (vibe):** при `vibe_id` и **`photo_app_config.vibe_attach_reference_image_to_generation`** = `true` (дефолт в миграции `sql/147_*.sql`) сервер качает `vibes.source_image_url` и шлёт в Gemini **два** изображения с метками **`VIBE_IMAGE_PART_LABEL_REFERENCE`**, референс, **`VIBE_IMAGE_PART_LABEL_SUBJECT`**, фото пользователя из Storage, затем **текстовую** часть. **Сборка текста** — **`assembleVibeFinalPrompt(rawPrompt, hasTwoImages)`**: порядок **(1)** тело сцены (`prompt_text` из клиента / expand), **(2)** один блок **CRITICAL RULES** (ветки **dual** vs **single**). Лог **`vibe_generation_layout`**: `architecture`, **`vibePromptChain`**, **`legacyPromptChain`**. Чтение: `getVibeAttachReferenceImageToGeneration` в `vibe-gemini-instructions.ts`.

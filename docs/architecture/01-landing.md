...
| `/api/imageprompt-proxy/extension/analyze` | Dev-only same-origin прокси к `imageprompt.tools/api/extension/analyze`. `getImagePromptAnalyzeUrl()` в `next dev` ведёт сюда (`/foto-v-promt` и blank-compose `intent=photo_prompt`); prod — `/api/extension/analyze`. |
...
- **Analyze site flow:** `/foto-v-promt` и blank-compose с `seed.intent=photo_prompt` (стартер «По фото» на `/generaciya-foto`)
  ...
  Композер шлёт только `image_base64` (`image-prompt-analyze-client.ts`); виджет `/foto-v-promt` по-прежнему умеет `image_url`.

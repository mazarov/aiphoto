...
| `/api/prompt-remix` | POST (auth + internal generation allowlist): локальный Gemini text-вызов для `originalPrompt + changeRequest`; модель `GEMINI_PROMPT_REMIX_MODEL` (default `gemini-2.5-flash`), proxy определяется через `photo_app_config.gemini_use_proxy` + `GEMINI_PROXY_BASE_URL` |

...
- **Robokassa UX:** официальный `robokassa_iframe.js` загружается один раз по требованию; `Robokassa.Render` получает подписанный сервером payload и `Settings={Mode:'modal'}`. `PaymentMethods` намеренно не ограничен: iframe показывает все способы, доступные магазину Robokassa. Платёжная форма открывается поверх PromptShot, `window.location.assign` не вызывается. `SuccessURL`/`FailURL` ведут на компактные `/payment/robokassa/*` внутри iframe.

...
| `/api/generation-config` | Конфиг генерации; `modality=image` (default) или `video`. Video: модели из `video_models`, 9:16/16:9, 4/6/8/10 сек, 720p, `enabled` из `video_animate_enabled` **или** allowlist (`azarov.maxim@gmail.com`). SSOT `lib/generation/image-options.ts` |
...
| `/api/generate` | Auth enqueue: image — **0–10** owned upload-фото без `editInstruction` (0 = text-only) или local edit (`parentGenerationId` + `editInstruction`). Video — `modality=video`, ровно одно фото **или** owned completed image parent, без edit; 4/6/8/10 сек / 720p / `gemini-omni-flash-preview`; кредиты = база + доплата за длительность; `create_ugc=false`. Fingerprint включает modality и duration. RPC `landing_enqueue_generation` (миграции **189**, **200**); ответ `202 { id, status: pending }` |
...
- **Video Interactions payload (Omni Flash):** SSOT `web-generation-worker/src/video-interaction.ts` → `buildVideoInteractionRequest`. Официальный unary: `background=false`, `store=false`, `stream=false`. `video_config` только `{ task: "image_to_video" }`; `aspect_ratio` и `duration` (`4s`–`10s`) в `response_format`. Gemini на этом task принимает **ровно одно** изображение — второе даёт `invalid_request` («Image-to-video does not support more than 1 image»). Текст — `assembleVideoMotionPrompt`: `[# Sources @Image1]` + identity lock на этот кадр. Submit логирует `httpStatus`, `errorCode`, `imageBytes`, `imageCount`, `linkedGenerationId`. Источник кадра: `video_input_resolved`. `user_photos` = аплоад; копия из «Использовать» (`generation-<uuid>.jpg` / `landing_user_photos.source_generation_id`, SQL **199**) поднимается в generation result как единственный Image1. `generation_result` = готовое фото. `background=true` на consumer Gemini даёт opaque `invalid_request`.

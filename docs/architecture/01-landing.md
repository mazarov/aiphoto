...
- **Video Interactions payload (Omni Flash):** SSOT `web-generation-worker/src/video-interaction.ts` → `buildVideoInteractionRequest`. `generation_config.video_config` содержит только `task: "image_to_video"`. `aspect_ratio` (`9:16` | `16:9`) — в `response_format`, не в `video_config` (иначе Google: `Unknown parameter 'aspect_ratio'`). `duration_seconds` в Interactions не отправляем.

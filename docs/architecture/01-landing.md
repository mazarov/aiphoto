...
- **Резолв:** `landing/src/lib/resolve-db-user-id.ts` — `landing_users`/`imageprompt_users` по JWT id → иначе `google_sub` из OAuth identity → иначе email. Используется в `/api/me`, `ensureLandingUserForGeneration`, list/get generations.

...
> Последнее обновление: 2026-08-01 (**feature/internal-generate-allowlist:** inline compose + `STV_OPEN_GENERATE_DEBUG` только для allowlist email (`azarov.maxim@gmail.com`, optional `INTERNAL_GENERATE_ALLOWLIST`); остальные пользователи — LexyGPT без STV drawer. Предыдущее: **client_source на генерациях:** `landing_generations.client_source` — всегда `site` при `POST /api/generate`; миграция `sql/168_landing_generations_client_source.sql`. Предыдущее: **feature/internal-generation-repeat:** inline compose на `/p/[slug]`; `STV_OPEN_GENERATE_DEBUG`. Предыдущее: STV web LexyGPT-style canvas + `STV_GUEST_MODE`.)
...
- **Условия:** флаг `STV_OPEN_GENERATE_DEBUG` (в development on по умолчанию; prod только `=1`) **и** session email в allowlist. Без allowlisted сессии bypass выключен → 401 / обычные credits.
- **Эффект:** generate с `creditsCharged=0` (без `landing_deduct_credits`); DB owner — `resolveGuestOwnerDbUserId` / `STV_GUEST_OWNER_USER_ID` при open-debug path.
- **Назначение:** временный обход для отладки inline compose под конкретным аккаунтом.

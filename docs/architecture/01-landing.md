...
> Последнее обновление: 2026-03-22 (STV expand: **full legacy style literal** — все непустые поля extract как помеченные блоки, без LLM; **legacy_2c23**; extension до 4 фото — `docs/23-03-stv-multi-user-photos-ui.md`)
...
- **Expand:** `POST /api/vibe/expand` — legacy **`style`** из body и/или строки vibe; **`vibeId`** + владелец + **`prompt_chain` = `legacy_2c23`** (иначе **404** / **409** как раньше). **Без text LLM:** **`mergedPrompt`** = **`buildLegacyVibeFullPromptBody(style)`** — все **непустые** поля из 8-field JSON как блоки `Label:\nтекст` (порядок: scene → … → composition); пустое тело → **400** **`missing_style_body`**. **`prompts`**: три элемента с **одинаковым** телом и акцентами `lighting` / `mood` / `composition` (triple-variant). **`finalPromptForGeneration`** = **`assembleVibeFinalPrompt(mergedPrompt, willAttachReferenceInline)`**. Ответ: **`mergeOk: false`**, **`modelUsed: "passthrough"`**, **`llmProvider: "none"`**.

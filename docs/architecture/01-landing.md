...
> Последнее обновление: 2026-09-24 (**тайтлы object-хабов:** эмодзи темы сразу после запроса, 🇷🇺 после «на русском». H1 без эмодзи.)
>
...
>
> Последнее обновление: 2026-09-11 (**compose style tile always on:** плитка «Выбрать стиль» не зависит от Фото/Видео/Фотосессии/Промт по фото. Прячется только с result chrome. Footer-гейт и auto-open шторки остаются image-only. SSOT `composeShowsExampleTool`.)
>
> Последнее обновление: 2026-09-11 (**compose photo/style tiles fill:** выбранные фото и стиль заливают весь квадрат плитки (`object-cover`). Несколько фото — мозаика на N ячеек (`composeTileMosaicGrid`). Плитка каталога — «Выбрать стиль», не «Пример». Пустое «Ваши фото» — иконка + `0/10`. SSOT `compose-tile-mosaic.ts` / `ComposeDockToolTile`.)
>
> Последнее обновление: 2026-09-11 (**compose dock tiles:** «Ваши фото» и «Выбрать стиль» — тот же chrome, что Фото/Видео: pill на верхнем бордере; выбранный — indigo fill. SSOT `ComposeDockToolTile`.)
>
> Последнее обновление: 2026-09-11 (**compose tool guides:** выбранные Фотосессии / Промт по фото показывают explainer в пустой верхней зоне модалки (как «Какое фото добавить»). Фотосессии: сначала одно фото, потом 2×2 из четырёх кадров. Промт по фото: empty-state `/foto-v-promt` (`FotoVPromtEmptyState` + `widgetCopy`). SSOT `compose-tool-guide.ts` / `ComposeToolGuide`.)
>
> Последнее обновление: 2026-09-11 (**compose tool tiles:** Фото / Видео / Фотосессии / Промт по фото — pill на верхнем бордере. Выбранная плитка: заливка indigo как у бордера (`indigo-400` glass / `indigo-500` light). SSOT `ComposeModeToolTile` / `composeTileBorder`.)
>
> Последнее обновление: 2026-09-11 (**compose example warmup on dock open:** первая страница «Выбрать пример» (`GET /api/listing?limit=12&sort=new`, плюс hub-фильтр) грузится в memory cache при idle listing и при маунте generate-панели. Пикер читает `peekComposeExampleListing` на первый кадр — без спиннера, если кэш/in-flight уже есть. Закрытие шторки не abort'ит warmup. SSOT `compose-example-audience-client.ts`.)
>
> Последнее обновление: 2026-09-11 (**compose: Photo/Video tiles show model:** плитки Фото/Видео — короткое имя + мини-лого; «Фото»/«Видео» pill на верхнем бордере как цена на карточках модели. Пустая фото-модель — «Выбрать». SSOT `displayTileLabelForGenerationModel` / `ComposeModeToolTile`.)
>
> Последнее обновление: 2026-09-11 (**compose: Pro badge «Русский текст»:** в шторке моделей (и showcase на той же карточке) у `gemini-3-pro-image-preview` emerald pill «Русский текст»; hint «Лучше читает русский текст на кадре». SSOT `compose-image-model.ts` / `ComposeModelChoiceCard`.)
>
> Последнее обновление: 2026-09-11 (**compose: no default image model:** первая фото-генерация не подставляет Nano Banana Flash. Модель пустая, пока пользователь не выберет её в шторке (как «Выбрать пример» после загрузки фото) или пока нет completed image job. После completed — restore `landing_generation_preferences` + session. Хаб `/nano-banana` больше не зовёт `preferModelId` для Flash; `/nano-banana/pro` по-прежнему предвыбирает Pro. SSOT `compose-image-model.ts`.)
>
...
>
> Последнее обновление: 2026-09-09 (**хаб `/promty-dlya-foto-devushki`:** тот же каркас, что хаб пар. SSOT `promty-dlya-foto-devushki-cluster.ts`. Диспетчер `listing-catalog-hub.ts` — одна развилка в `[...slug]/page.tsx` (`resolveListingCatalogHubL1`), один 301-предикат в middleware/sitemap. Title/H1 «промты для фото девушки» + `7000+`; intro «женских фото»; H2 «промты для ИИ фотосессии женские». HowTo на хабе нет (`howToSteps: []`, `SeoHowToSection` не рисует пустую карточку). Карусель `audience_tag=devushka` `sort=new`, query-фильтр в hero не протекает. L2 `/promty-dlya-foto-devushki/*` → 301 на хаб, кроме хвостов `den-rozhdeniya` (кластер ДР / `next.config`). `/ii-fotosessiya/zhenskie` не трогаем. `popularLinks` / `seoTextBlocks` / mid-page generate-CTA нет. FAB «Создать фото девушки» → `intent=text` + example + чип `Девушки`.)
>
> Последнее обновление: 2026-09-08 (**хаб пар: H2 над поиском:** `seo.explorerTitle` / `explorerIntro` → `CatalogWithFilters` как `ListingExplorerHeading` h2 (`listing-explorer-gallery-heading`). Запрос блока — «промт для парной фотосессии»; hero H1/intro держат «парных фото» / «фото пары», без пересечения. Это не `seoTextBlocks`.)
>
> Последнее обновление: 2026-09-08 (**хаб пар = шаблон главной:** `/promty-dlya-foto-par` рендерится через `GeneraciyaFotoHeroPage` (тот же chrome, что `/`: open hero H1 + lead + `GeneraciyaFotoHeroCarousel` + stack). Карусель не в `CatalogExplorer`. Карточки hero — `audience_tag=para` `sort=new` (`pairsHubHeroFetchParams`), `ctaLabel=null`. Листинг ниже с `hideHeading`. `/` тоже переведён на этот shell.)
>
> Последнее обновление: 2026-09-08 (**хаб пар: сняты оба SEO-текстовых блока:** на `/promty-dlya-foto-par` больше нет `seoTextBlocks`. HowTo и FAQ остаются. Exact-match «с парнем» — Title, description, FAQ и чип-фильтр.)
>
> Последнее обновление: 2026-09-08 (**listing: снята плашка «Промт не попадает в фото?»:** `ListingFotoVPromtBanner` / `FotoVPromtMiniBanner` больше не монтируются на `[...slug]`, `/search`, `/sobytiya/1-sentyabrya` (desktop + mobile). Компоненты и copy/metrics удалены. Цели `foto_v_promt_banner_click` / `_card` — leftover, не вызываются.)
>
> Последнее обновление: 2026-09-08 (**хаб `/promty-dlya-foto-par` — FAB вместо mid-page generate-CTA:** блок `SeoGenerateCtaSection` с хаба снят. Idle FAB / mobile tab = `Создать фото пары` (`listingGenerateIdleCta`). Клик сидит `intent=text` + `dockSurface=example`, чтобы не поднимать last-result chrome и сразу открыть «Выбрать пример» с чипом `Пары` (`audience_tag=para`, SSOT `PAIRS_HUB_COMPOSE_EXAMPLE_FILTER`).)
>
> Последнее обновление: 2026-09-08 (**listing SEO = homepage GF template:** HowTo / FAQ / SEO-текст / related / generate-CTA на `[...slug]`, `/trends`, `/sobytiya/1-sentyabrya` и главной — один каркас `SeoPageSections` + `GeneraciyaFotoFaqBlock` (`GF_BLOCK`, `GF_H2`, шаги `01/02`, pill-чипы). На хабе пар mid-page generate-CTA нет: вход через FAB `Создать фото пары`.)
>
> Последнее обновление: 2026-09-08 (**low-balance 24h reuse:** грант `low_balance_upgrade` не закрывается на credit и не на второй покупке. −20% действует все 24 часа и на каждую кассу в этом окне. Письмо после второй live-оплаты по-прежнему отменяется. Живые уже списанные гранты внутри TTL открываются в SQL `251`. Новый грант по-прежнему только после первой 99 ₽.)
>
...
>
> Последнее обновление: 2026-09-04 (**compose example child audience:** соло-ребёнок больше не `null`. Классификатор пишет `malchik` / `devochka` / `malysh` и фильтрует «Выбрать пример» (`strict=1`). Взрослый пол для ребёнка не используем. SQL `241` расширяет CHECK и сбрасывает кэш `audience_tag IS NULL`. Спека `docs/04-09-compose-example-match-photo.md`.)
>
> Последнее обновление: 2026-09-04 (**compose example picker first page:** шторка «Выбрать пример» берёт `GET /api/listing?limit=12` (первый экран 3×4 / 4×3), дальше sentinel. Картинки первой страницы `eager`, хвост `lazy`. Поиск debounce 200ms. Не меняет размер страниц каталога / birthday search.)
>
> Последнее обновление: 2026-09-04 (**compose example match by audience:** после identity-фото шторка «Выбрать пример» остаётся на `GET /api/listing?sort=new`, затем (флаг `compose_example_match_enabled`) Gemini Flash JSON классифицирует **кто на фото** → `audience_tag=devushka|muzhchina|para|semya|malchik|devochka|malysh` и подменяет listing (`strict=1`, сцена-чип AND). Не visual k-NN. Чип тега с крестиком сбрасывает на newest. Поиск `q=` снимает auto-тег. Кэш на `landing_user_photos.audience_tag`, не face-vector. `POST /api/compose/classify-audience`. SQL `240`+`241`. Спека `docs/04-09-compose-example-match-photo.md`. SSOT: `compose-example-audience.ts`.)
>
> Последнее обновление: 2026-09-04 (**«Повторить» → плитка «Выбрать пример»:** `catalogCardRepeatSeed` пишет `cardId` + промт + https-превью карточки в `GenerateDockSeed`. Карточка (`CardPageClient`) и hover листинга (`PromptCard`) зовут один `seedFromCard`. Плитка берёт `seed.examplePreviewUrl`, не пустой плейсхолдер. Видео-repeat по-прежнему `intent=animate`.)
>
> Последнее обновление: 2026-09-04 (**«Выбрать пример» = image compose, не URL:** плитка, шторка и footer-гейт живут в `composeShowsExampleTool` / `composeNeedsExamplePick` / `composeShouldAutoOpenExampleSheet`. Гейт — `composeMode===image` и не result chrome; `generationSurface` / pathname / `seo_page` не фильтруют. `/foto-v-promt` (`photo_prompt`), видео и фотосессия без плитки. API `generationSurface` остаётся меткой воронки.)
>
> Последнее обновление: 2026-09-04 (**OAuth не consume pending на `/auth/callback`:** `GenerateDockProvider` / restorer не читают `promptshot:pending-generate-dock` на `/auth/*` — иначе full `location.replace` на listing открывает пустой seed. Каталожный пример дублируется в `promptshot:pending-compose-example`; persist selfie без `cardId` sidecar не стирает. `seedForAuthReturnDock` мержит sidecar. Фото после логина — `landing_user_photos`. `/foto-v-promt` без изменений.)
>
> Последнее обновление: 2026-09-04 (**guest catalog example survives auth:** выбор «Выбрать пример» пишет `cardId` + промт + https-превью в `GenerateDockSeed` без remount (`applyComposeExamplePick`). Гость — `promptshot:pending-generate-dock`. Selfie-стартер не затирает уже выбранный образ (`applySeoSelfieToSeed`). `/foto-v-promt` без изменений.)
>
> Последнее обновление: 2026-09-04 (**authed identity photo → library:** selfie в compose (`attachIdentityPhoto`, плитка «Ваши фото») после логина пишется в `landing_user_photos` через `POST /api/upload-generation-photo` `saveToLibrary=true`. Гость остаётся ephemeral data URL. Hydrate больше не затирает только что загруженный ряд stale GET (`mergeLibraryHydratePhotos`, dep `user?.id`). Enqueue не шлёт пустой `storagePath`. `/foto-v-promt` / `photo_prompt` по-прежнему без библиотеки. SSOT: `compose-library-photos.ts`.)
>
> Последнее обновление: 2026-09-04 (**SEO example picker chips СВО + Осень:** закреплённые чипы шторки — `object_tag=v_forme` и `object_tag=osen` (`/osen`). Хаб `/generaciya-foto` scenarios не меняли.)
>
> Последнее обновление: 2026-09-04 (**SEO example picker chip «СВО»:** первый чип шторки «Выбрать пример» — `object_tag=v_forme` (каталог «В форме»: военная форма / солдат), `/api/listing?strict=1`. Дубль чипа «В форме» в пикере не показываем. Хаб `/generaciya-foto` scenarios не меняли.)
>
> Последнее обновление: 2026-09-04 (**SEO selfie → example sheet:** после загрузки фото на `seo_page` сразу `dockSurface=example`, если compose ещё без `cardId` с листинга (Repeat / карточка). Стартер `seedSeoSelfieCompose` открывает шторку сразу; upload в «Ваши фото» — тот же хелпер `seoComposeShouldAutoOpenExampleSheet`. Уже выбранный образ (listing или picker) не переоткрывает.)
>
> Последнее обновление: 2026-09-04 (**SEO idle dock = compact bottom plate:** `seoIdleCompose` больше не форсит `dockTall`. Idle `/generaciya-foto` — нижняя плашка как `/generate`; шторка «Выбрать пример» (`dockSurface=example`) по-прежнему растягивает plate через `editorOpen`.)
>
> Последнее обновление: 2026-09-04 (**SEO compose example = tool sheet:** поиск и сетка каталога больше не занимают колонку idle-модалки. На `seo_page` + image после «Ваши фото» плитка «Выбрать пример»; клик открывает шторку (`dockSurface=example`): поиск, чипы сценариев, listing, CTA «Выбрать». Промт/cardId пишутся по confirm. Footer-гейт «Выбрать пример» открывает ту же шторку. `/foto-v-promt` без изменений. SSOT: `generaciya-foto-compose-example.ts`, `ComposeExamplePicker`.)
>
> Последнее обновление: 2026-09-04 (**SEO compose picker chrome:** чипы «Фотосессия»/«Видео» сняты — только newest stills `sort=new`. Lead «Промт подставим…» убран. Плитки «Ваши фото» и инструменты в SEO-picker — `h-16` (64px), не 5.25rem. Сетка по-прежнему `min-h-0` над промптом/CTA.)
>
> Последнее обновление: 2026-09-04 (**SEO compose example picker layout:** сетка скроллится в `min-h-0` зоне **над** промптом / плитками / «Войдите», не уезжает под них. Дефолт — `/api/listing?sort=new` (дата добавления). Чипы «Фотосессия» / «Видео» — client filter `isPhotoshoot` / `videoUrl` на том же listing, без нового API; для sparse kind до 60 + drain 3 страницы. Заголовки карточек и дубль «Образ: Visual Hook:…» сняты. Выделение — inset `after:border-2` (внешний ring клипался `overflow-hidden`). Догрузка по sentinel, `ranked_batch_size`.)
>
> Последнее обновление: 2026-09-04 (**`/generaciya-foto` theme collage = 6 photos:** блок «Подборки шаблонов по темам» больше не режет коллаж до 4 кадров. Хаб вызывает тот же `fetchNewestThemeCollagePhotos` / `buildThemeCollageFromNewestResults`, что и главная: до 6 уникальных URL, layout `GeneraciyaFotoThemeCollage` уже 6 слотов. 22 темы × `resolve_route_cards` как раньше; лимит выборки 10 (запас на дубли URL).)
>
> Последнее обновление: 2026-09-04 (**`/generaciya-foto` selfie → example picker:** SEO-воронка (`generationSurface=seo_page`) разделяет identity-фото и образ из каталога. Стартер «С вашим фото» (`GeneraciyaFotoStarter`) больше не запускает `photo_prompt` analyze — `seedSeoSelfieCompose` открывает idle compose (`composeMode=image`, `attachIdentityPhoto`), без шторки photos. Блок «Выберите образ» — первый экран модалки: поиск + чипы + сетка (`ComposeExamplePicker`), `/api/listing` (CDN 60s/SWR 300s). Промт из listing payload; `/api/card` — fallback. Photos sheet снова только гайд селфи. Без `cardId` и короткого промта CTA = «Выберите образ» (фокус в поиск). Генерация = user photo + текст карточки. `/foto-v-promt` без изменений. SSOT: `generaciya-foto-compose-example.ts`.)
>
> Последнее обновление: 2026-09-02 (**`/generations` first paint:** кабинет больше не ждёт hydrate + CSR `GET` и не качает 2K оригиналы в сетку. RSC `force-dynamic` читает первую страницу через `landing_list_my_generations` (SQL `239`: два index-backed UNION, slim columns). Ответ — `resultUrl` / `photoshootTileUrls` (оригинал для dock/download) + `resultThumbUrl` / `photoshootTileThumbUrls` (listing 512/q58) + `photoshootSheetUrl` (внутренний лист). Сетка сначала пробует 4 кадра; 404 → показывает лист, а не битые иконки. Карточка = `next/image` + `PhotoshootListingGrid` + in-view `ListingCardVideo`; `/generate` остаётся client fetch. Fallback `.or(requester, legacy)` если RPC нет в schema cache.
>
> Последнее обновление: 2026-09-02 (**photoshoot planner temperature:** слайдер креативности снят. Planner всегда `temperature=2` (`PHOTOSHOOT_PLANNER_TEMPERATURE`, Gemini 2.5 Flash max) и bold-editorial brief. Клиентский `plannerTemperature` и числа в старых `edit_instruction` игнорируются. Fingerprint всегда 2. I2I-модель temp не видит.)
>
> Последнее обновление: 2026-09-02 (**photoshoot cabinet tiles:** `/generations` и poll не ждут колонку `photoshoot_tile_paths`. `resolvePhotoshootUserFacingResult` восстанавливает sidecar `lease-1.jpg`…`lease-4.jpg` из листа. Complete RPC пишет тайлы атомарно (SQL `238`). Пустой `completed` больше не остаётся «Готово…».)
>
...
>
> Последнее обновление: 2026-09-08 (**хаб `/promty-dlya-foto-par` — кластер «с парнем» без переспама:** Title `Промты для фото с парнем и парных фото — 800+ идей`, широкий H1 `Промты для парных фото`; точный объём вынесен в `ListingPromptCountBadge`, поэтому при `?audience=s_parnem` H1 не обещает 800+ результатов. Exact-match остаётся в Title, description, одном FAQ и чипе-фильтре; удалён из intro, HowTo, отдельного H2 и повторяющихся абзацев. `/promty-dlya-foto-s-parnem` даёт точный 301 на хаб и исключён из sitemap/меню; главная ведёт сразу на `?audience=s_parnem`, без внутреннего перехода через 301. Стили и аудитории — in-page фильтры `getPairsHubFilterNavItems`, canonical остаётся хаб. Листинг как `/generaciya-foto`: fade + кнопка `Больше промтов для пар` (`teaserLoadMore` в `InfiniteGrid`), дальше «Показать ещё», без sentinel. `popularLinks` хаба пустые; mid-page generate-CTA снят — вход в генерацию через FAB `Создать фото пары` → шторка «Выбрать пример» с фильтром `Пары`.)
>
...
>
> Последнее обновление: 2026-09-02 (**`/ii-fotosessiya` hybrid SEO owner:** хаб и 17 L2 совмещают сервисный интент ИИ-фотосессии с кластером готовых промтов. Старые `/promty-dlya-ii-fotosessii*` сохраняют 301 на соответствующие новые URL, не на главную. Hero сразу открывает загрузку фото; после сценариев SSR-рендерятся до 6 уникальных русских промтов. Галерея, промты, коллажи, noindex-порог 8 и sitemap используют только опубликованные многофайловые фотосессии из completed jobs/web UGC + SEO-тег сценария. Главная сохраняет широкий кластер «промты для фото» и передаёт вес ссылкой на хаб. SSOT: `photoshoot-listing.ts`, `promty-dlya-ii-fotosessii-page-data.ts`, `promty-dlya-ii-fotosessii-seo-copy.ts`.)
>
> Последнее обновление: 2026-09-02 (**`/generaciya-foto` SEO intent + L2 conversion:** Title хаба = «Сделать фото ИИ онлайн по фото или описанию»; H1 остаётся owner-запросом. Хаб явно обещает один кадр, показывает условия входа/кредитов и не рендерит синтетические keyword-stuffed отзывы. Видимый FAQ сокращён до 15 вопросов. SSR: общий пул и L2 загружают 24 карточки, hero — 12; коллаж темы — до 4 фото. Сырые EN `Visual Hook` в alt/aria заменяются русским контекстным fallback. Все 22 `/generaciya-foto/{scenario}` получили `GeneraciyaFotoStarter` с предзаполненным тематическим промтом, CTA hero ведёт к starter; видимый блок связывает один кадр с `promptCatalogHref` / ИИ-фотосессией и хабом. Sitemap больше не ставит ложный `lastModified=now` страницам без фактической даты; карточки сохраняют реальный `updated_at`. Топ-6 L2 добавлены в SEO watchlist. SSOT: `generaciya-foto-seo-copy.ts`, `generaciya-foto-scenario-copy.ts`, `GeneraciyaFotoStarter.tsx`.)
>
> Последнее обновление: 2026-09-02 (**`/nano-banana` SEO reanalysis:** Title таргетирует бренд + Google + фото онлайн; хаб не забирает самостоятельные кластеры Pro / 2 / промты. Порядок блоков: hero + starter → модели → примеры с тематическими чипами → инструменты → HowTo → доступ из России → тарифы → FAQ. Удалены отдельная карусель тем, синтетические SEO-отзывы и ранний cross-sell; сервер больше не собирает collage тем. `NanoBananaAccess` закрывает без VPN, русский интерфейс, рубли и distinction от Google AI Studio. Страница скрыта из сайдбара и hero-menu главной; блок моделей `/generaciya-foto` использует те же компактные chips с логотипом провайдера, а все карточки семейства Nano Banana являются индексируемыми ссылками на `/nano-banana`. Тихий default `preferModelId(gemini-2.5-flash-image)` не блокирует смену модели. Спека `docs/02-09-nano-banana.md`, SSOT `nano-banana-seo-copy.ts`.)
>
> Последнее обновление: 2026-09-02 (**Robokassa parent redirect guard:** `robokassa_iframe.js` на `postMessage action=redirect` делает `window.location.href`. Страница `Merchant/State` уводила вкладку с PromptShot, после чего «Вернуться в магазин» писала в пустой parent. Capture-listener глушит State/SuccessURL redirect: State остаётся в iframe, Success/Fail закрывает overlay. 3DS на чужой host не трогаем.)
>
> Последнее обновление: 2026-09-02 (**Robokassa return to store:** «Вернуться в магазин» грузит Success/Fail URL внутрь iframe и раньше оставляла оверлей. `/payment/robokassa/*` закрывает `robokassa_iframe` в parent или делает `location.replace` на экран до оплаты (`sessionStorage`). Poll `succeeded` закрывает тот же overlay. SSOT: `robokassa-return.ts` + `closeRobokassaOverlay`.)
>
...
>
> Последнее обновление: 2026-09-01 (**remix save ≠ generate:** шторка «Что изменить?» CTA = `Сохранить`. Remix только переписывает промпт и закрывает шторку. Генерация — отдельно: footer «Создать фото» или rail после save. Спека `docs/01-09-remix-save-only.md`.)
>
...
>
> Последнее обновление: 2026-08-31 (**I2V prompt ≠ catalog extract:** Repeat/Grok 400 — в submit уходил 12-блочный фото-промпт. SSOT `videoI2vUserPrompt` в `video-motion-prompt.ts` (worker + card Repeat): только `Motion:` или короткий beat, clamp 400. Каталог по-прежнему показывает look.
>
...
>
> Последнее обновление: 2026-08-31 (**photos sheet: library on the frontend:** idle `prefetchUserPhotoLibrary` + sessionStorage. Шторка «Ваши фото» сразу показывает гид и прибитый вниз ряд «Добавить» / превью / «Готово»; hydrate больше не прячет гид и не поднимает кнопки. Спека `docs/31-08-generate-dock-instant-compose.md`.
>
> Последнее обновление: 2026-08-31 (**generate-dock instant compose:** внешний CTA (`tab` / `fab` / `howto` / `hero`) для `photoshoot` и `photo_prompt` сразу ставит чип из `composeModeFromDockIntent` и `dockSurface: "photos"`. Повтор с тем же intent не бампает `seedToken`. На `/ii-fotosessiya*` и legacy `/promty-dlya-ii-fotosessii*` плитка «ИИ фотосессия» optimistic до config (`optimisticPhotoshootEnabled`). Idle prefetch чанка + `/api/generation-config` — `scheduleGenerateDockPrefetch`. Подпись таббара — `listingGenerateIdleCta` ← `listingGenerateIdleIntent`: `/foto-v-promt` «Создать промт по фото», fotosessii «Создать ИИ фотосессию», остальное «Создать фото». Гость без фото на photoshoot: «Загрузите фото». Enqueue из библиотеки без parent — SQL `230`. Спека `docs/31-08-generate-dock-instant-compose.md`.
>
> Последнее обновление: 2026-08-31 (**хаб H1 без «своему»:** `/ii-fotosessiya` H1 `ИИ фотосессия по фото`. Title без изменений. Copy SSOT `promty-dlya-ii-fotosessii-seo-copy.ts`.
>
> Последнее обновление: 2026-08-31 (**хаб ИИ-фотосессии → `/ii-fotosessiya`:** исходный коммерческий кластер «ии фотосессия / по фото». L2 `/ii-fotosessiya/{slug}`. 301 `/promty-dlya-ii-fotosessii` и `/:slug` + `/promty-dlya-fotosessii` → новый хаб. Актуальная спека `docs/done/31-08-ii-fotosessiya.md`.
>
> Последнее обновление: 2026-08-30 (**card «Повторить» guest = auth overlay:** гость на карточке не открывает generate-dock. SSOT `resolveCardRepeatAction` / `beginGuestCardRepeatAuth`: ждать hydrate → модалка входа, карточка остаётся, `ps_ov=card:<slug>` на листинг (не `/p/slug`). Authed → `seedFromCard` + уход на last listing, **не** `history.back()` (после OAuth в истории может быть `oauth.yandex.ru`). Live overlay `card` не затирается `generate-dock`. GoTrue `error` / `error_code` / `error_description` снимаются из return path.
>
...
>
> Последнее обновление: 2026-08-30 (**unpaid YooKassa bar:** `UnpaidCheckoutBanner` над навбаром на 24 ч после latest create без `credited_at`. Крестик — localStorage на `payment_id`. С 15-й минуты фаза flash, если жив грант 25% (`yk_abandon_5m`, TTL 60 мин с письма / `created_at+65м`). Countdown = `offer.expires_at`, не второй час. CTA «Продолжить» → новый create (ЮKassa redirect или Robokassa iframe). `GET /api/payments/unpaid-banner` без reconcile; `/api/payments/yookassa/unpaid-banner` — alias. Спека `docs/30-08-yk-unpaid-banner.md`, обобщение `docs/01-09-robokassa-traffic-split.md`.
>
...
>
> Последнее обновление: 2026-08-29 (**compose CTA / tiles:** превью фото только на плитке «Ваши фото». Плитки Фото / Видео / Фотосессия — иконка + подпись режима, без фото и без glow-цены. Footer: «Создать фото / видео / фотосессию»; имя выбранной фото/видео-модели + стоимость — на этой кнопке (`composeGenerateCtaLabel`).
>
> Последнее обновление: 2026-08-29 (**fotosessii hub copy:** лиды hero / `#temy` / `#howto` / `#primery` / session + шаги HowTo. FAQ без изменений. Под H2 «Тарифы» на хабе — `pricingLead` (`PricingScreen` embed `lead`, `/generaciya-foto` без лида).
>
> Последнее обновление: 2026-08-29 (**fotosessii HowTo:** с хаба снята кнопка «Купить кредиты»; остаётся «Выбрать пример» → `#primery`.
>
> Последнее обновление: 2026-08-29 (**fotosessii hero:** с хаба сняты кнопки «Скопировать промт» / «Собрать кадр»; карусель — «Выберите стиль ИИ фотосессии» → скролл на `#primery`; вход в генерацию — FAB «Создать ИИ фотосессию».
>
> Последнее обновление: 2026-08-29 (**fotosessii FAB:** на `/promty-dlya-ii-fotosessii` и L2 детях idle-кнопка generate-dock — «Создать ИИ фотосессию» / «Войти и создать ИИ фотосессию» (`listingGenerateIdleCta`). Остальные листинги без изменений.
>
> Последнее обновление: 2026-08-29 (**compose tile clicks:** «Фото» и «Видео» открывают шторку своей модели (`composeModeTileSheet` / `nextComposeModeTileSheet`); повторный клик закрывает. «Фотосессия» только выбирает режим (`aria-pressed` + border), шторок и overlay не открывает. Overlay — footer CTA / rail. Выбранная фото-модель: имя + цена + glow-бейдж на плитке. Лого моделей меньше (`h-4`). Выбранный бордер — `::after` (`border-2`): `OVERLAY_BUTTON_UA_RESET` глушит `border`/`ring`, а `overflow-x-auto` режет outset.
>
> Последнее обновление: 2026-08-29 (**photoshoot from library:** плитка «Фотосессия» в compose берёт ровно одно выбранное фото из «Ваши фото» (`resolvePhotoshootLibraryFrame`), не last image / parent. Overlay без фейкового `generationId`. Enqueue: `editKind=photoshoot` + одно owned `photoStoragePaths`, `parent_generation_id` null. API parent XOR одно upload; `photoshoot_busy` по parent или тому же `input_photo_paths`. Rail после готового image — по-прежнему parent. Worker принимает `user_photos`.
>
> Последнее обновление: 2026-08-29 (**generate compose modes:** в `CardInlineGeneratePanel` три взаимно исключающих режима — фото / видео / фотосессия (`composeMode`, SSOT `lib/generate-compose-mode.ts`). Видео вынесено из шторки фото-моделей в свою плитку; выбранный режим — `aria-pressed` + indigo ring, без бейджа «Видео» на фото-плитке. Фотосессия — кнопка, не третья шторка: тот же `PhotoshootOverlay` + `editKind=photoshoot`. Compose SSOT = выбранное фото в «Ваши фото»; rail после image result — parent generation. Флаги `video_animate_enabled` / `photoshoot_enabled`.
>
> Последнее обновление: 2026-08-29 (**L2 ИИ-фотосессии + ч/б:** 17 детей. Новый `/cherno-belye` (`style_tag=cherno_beloe`), каталог одного кадра `/stil/cherno-beloe`.
>
> Последнее обновление: 2026-08-29 (**L2 ИИ-фотосессии:** 16 детей хаба в порядке меню — `muzhskie`, `zhenskie`, `pary`, `den-rozhdeniya`, `detskie`, `semeynye`, `studiynye`, `zimnyaya`, `beremennye`, `s-voennymi`, `dlya-dvoih`, `novogodnyaya`, `vesennie`, `delovoy-stil`, `nyuborn`, `s-mashinoy`. «Для двоих» = `vlyublennykh`, не дубль `pary`. Occasion «день рождения» в каталоге остаётся `/sobytiya/den-rozhdeniya`.
>
> Последнее обновление: 2026-08-29 (**хаб `#primery`:** лента луков из `resolve_route_cards`, без фильтра «только фотосессия». Карусель hero — только опубликованные фотосессии (`photoshoot-listing.ts`).
>
> Последнее обновление: 2026-08-29 (**хаб ИИ-фотосессии:** ключ «промты для ии фотосессии в нейросетях». Порядок: hero → аудитории → HowTo → примеры → тарифы → FAQ. Карусель — карточки-фотосессии. `#primery` — обычные луки. Блока «Собрать кадры на PromptShot» нет.
>
> Последнее обновление: 2026-08-28 (**кластер ИИ-фотосессии:** хаб `/promty-dlya-ii-fotosessii` + L2 `zhenskie` / `pary` / `muzhskie` / `semeynye` / `detskie` / `beremennye`. Каркас GF без tools/тарифов/отзывов. Ключ «промты для ии фотосессии» снят с `/`. 301 `/promty-dlya-fotosessii` → хаб. Спека `docs/28-08-promty-dlya-ii-fotosessii.md`.
>
> Последнее обновление: 2026-08-29 (**AI-фотосессия mobile card:** шильд «Фотосессия» компактный (`11px`). На `/p` сетка 2×2 выше нижнего стекла; тап по кадру раскрывает его на весь экран + плёнка 4 миниатюр и кнопка сетки. Тап плитки в листинге — press-scale и zoom-in модалки.
>
...
>
> Последнее обновление: 2026-08-28 (**`/generations` пагинация:** клиент больше не режет историю на первые 50. `GET /api/generations` peek `limit+1` → `hasMore` / `nextOffset`. Страница грузит по 24 и дозаполняет viewport тем же `useListingSentinelLoadMore`, что каталог. Poll/focus мержит первую страницу, не затирает уже подгруженный хвост.
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия модель:** `photoshoot_model` = `seedream-5.0-pro` (SQL `227`, дефолт в коде тот же). Цена job по-прежнему `PHOTOSHOOT_CREDIT_COST` 15 кр. Seedream 4.5 в пикере скрыт — на него не ставим, иначе 503.
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия очередь:** job идёт в ту же `landing_enqueue_generation` / `landing_claim_generations` (image, `max_per_user=3`). `photoshoot_busy` только на том же `parent_generation_id`. «Выйти» во время съёмки не отменяет job — poll обрывается, можно поставить следующую.
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия CTA:** rail две строки «Фотосессия» / «4 фото», шильд `15✦` сверху справа (`GenerationCreditCostBadge`). `PHOTOSHOOT_CREDIT_COST=15` на enqueue, не cost `photoshoot_model`.
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия 2K:** `POST /api/generate` для `editKind=photoshoot` всегда ставит `image_size=2K` (`PHOTOSHOOT_IMAGE_SIZE`), не качество родителя. Grok clamp 4K→2K на эту ветку не нужен.
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия креативность:** клик «Фотосессия» открывает правый rail сверху вниз — «Выйти», ползунок с живой подсказкой (нейтральнее / нейтрально / смелее / невероятные сюжеты), «Создать». UI 50 = temp 0.5, 100 = temp 2.0. Worker меняет brief планера; I2I-модель температуру не видит.
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия history card:** `/generations` — бесшовный 2×2 `object-cover`, шильд «Фотосессия» снизу по центру (`PhotoshootListingBadge`). Тот же шильд на UGC-листингах (`ListingPhotoTile` / `PromptCard`). Desktop hover подсвечивает кадр; клик открывает этот кадр в result chrome. Follow-up берёт тайл, не лист. SQL `226`.
>
> Последнее обновление: 2026-09-02 (**AI-фотосессия user-facing:** лист 2×2 не отдаём клиенту. `GET /api/generations` и `/:id` ставят `resultUrl` = первый тайл, `photoshootTileUrls` = 4 кадра (`resolvePhotoshootUserFacingResult`; если колонка пуста — sidecar от `result_storage_path`). Оверлей без «Скачать лист» и без CSS crop. История и каталог — сетка 4 кадров. UGC publish — **одна** `prompt_cards` + 4 `prompt_card_media` (тайлы, не лист, не `card_split`) + 4 analyzed `prompt_variants`. Старый черновик с одним кадром дописывается на ensure/publish. save-to-library — тайл 1. Split fail валит job. SQL `238`: тайлы в `landing_complete_generation`.
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия промпт листа:** `Canvas {aspect}` только выбранный холст (не меню `1:1, 16:9, or 9:16`). Панели встык: `PHOTOSHOOT_FLUSH_PANELS_RULE` — без gutter / white seam / grid. Тот же текст в Gemini и Grok/Seedream-сборщиках.
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия aspect:** вход снапается в холст `1:1` / `16:9` / `9:16` (`resolvePhotoshootSheetAspect`: 4:3/3:2→16:9, 3:4/2:3→9:16). Job и 2×2 лист в этом формате; клетка после `sharp.extract` тот же ratio. Спека `docs/28-08-ai-photoshoot-aspect.md`.
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия split:** после I2I worker режет лист `sharp.extract` 2×2 → 4 JPEG sidecar (`lease-N.jpg`), пути в `photoshoot_tile_paths` (SQL `225`, complete — `238`). Poll отдаёт `photoshootTileUrls`; пустая колонка → sidecar от листа. Спека `docs/28-08-ai-photoshoot-split.md`.
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия:** rail «Фотосессия» после готового image. `POST /api/generate` `editKind=photoshoot`, 1 job / 15 кр (`PHOTOSHOOT_CREDIT_COST`), модель `photoshoot_model` (дефолт Grok). Worker: vision planner → 2×2 sheet. Planner = Gemini 2.5 Flash + `thinkingBudget: 0` + `responseSchema` (`PHOTOSHOOT_PLANNER_PROMPT_V2`); иначе thinking съедает 1024 токена и JSON не парсится. Флаг `photoshoot_enabled` (SQL `224`). Плёнка 4 тайла. `Dockerfile.worker` копирует `photoshoot.ts` + `photoshoot-planner.ts`. Спека `docs/28-08-ai-photoshoot.md`.
>
...
>
> Последнее обновление: 2026-08-27 (**listing query vectors:** birthday SSOT `q` хранят Gemini query embed в `listing_query_embeddings` (SQL `223`). Listing читает вектор до Gemini; miss → live 8000 мс + upsert. Cron `visual-embeddings` греет только эти запросы. `/api/search` по-прежнему live 800 мс, в таблицу не пишет.
>
> Последнее обновление: 2026-08-27 (**listing embed timeout:** birthday search L2 (`searchListingCardsHybrid`) ждёт query embed **8000 мс** (`LISTING_HYBRID_EMBED_TIMEOUT_MS`). `/api/search` остаётся на 800 мс. RF→DO proxy на query embed ≈2 с в покое и >2.5 с на SSR; 800/2500 мс резали visual branch → AND-FTS из 8 карточек на `/muzhchiny`.
>
> Последнее обновление: 2026-08-27 (**birthday hub = category tags:** `/sobytiya/den-rozhdeniya` снова `resolve_route_cards` (`occasion_tag=den_rozhdeniya`), сорт по новизне как у девушке/детям/с тортом. Поиск только у слабых L2. L3 301 на audience L2.
>
> Последнее обновление: 2026-08-27 (**birthday L2 split:** девушке / детям / с тортом — `resolve_route_cards`. Мужчине / с детским фото / с шампанским / со львом — поиск (`мужской день рождения`, `день рождения с детским фото`, `с шампанским`, `со львом`). Хаб — `день рождения`. L3 301 на audience L2.
>
> Последнее обновление: 2026-08-27 (**birthday L2 = category tags:** L2 `/sobytiya/den-rozhdeniya/{alias}` снова `resolve_route_cards` (AND `seo_tags`), не FTS. Поиск + hybrid 500 только у хаба. L3 по-прежнему 301 на audience L2. Спека `docs/27-08-birthday-l3-retire-search-window.md`.
>
> Последнее обновление: 2026-08-27 (**birthday L3 retire + L2 search window:** L3 `/sobytiya/den-rozhdeniya/{audience}/{object}` — 301 на audience L2 (object-first → audience L2, object×object → первый L2). Сетка хаба/L2: запрос `день рождения` + `audience_tag`/`object_tag`, не AND-фразы. Listing hybrid материализует 500 (SQL `222` `least(500, p_limit)`); `/api/search` остаётся на окне 100. Спека `docs/27-08-birthday-l3-retire-search-window.md`.
>
...
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` тарифы:** оферта / почта / реквизиты — одна строка под карточкой, слева. Внутри paywall больше нет.)
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` тарифы:** пакеты по возрастанию цены: 99 → 299 → 469 → 990. Модалка `/pricing` без изменений (swipe-порядок).
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` ритм:** одна шкала карточек/H2/лида/CTA (`generaciya-foto-ui.ts`), равные отступы между секциями.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` тарифы:** embed без hero-фото и слогана «Теперь фотосессия — это просто»; ширина = колонка страницы, 4 пакета в ряд на lg.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` модели UI:** плашки glass как на темах, CTA indigo-градиент `rounded-full`, без белой кнопки и чипа «Фото».
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` темы:** ширина карточек «Подборки шаблонов по темам» = колонка masonry «Идеи для фото ИИ» (2 / 3 / 4).
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` модели:** ширина карточек = колонка masonry «Идеи для фото ИИ» (2 / 3 / 4).
>
> Последнее обновление: 2026-08-26 (**сайдбар:** пункт `/generaciya-foto` — «Сделать фото», без «ИИ». Хлебные крошки и чип хаба без изменений.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` тарифы:** после моделей блок `#tarify` = compact paywall варианта B (`treatment`), не A/B. CTA → ЮKassa, return `/generaciya-foto`. `PricingScreen variant=embed`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` модели:** вернули исходные тёмные фотокарточки; тексты по-прежнему из `GENERATION_MODEL_DISPLAY`. Сетка → горизонтальная карусель (как отзывы).
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` отзывы:** `#otzyvy` — горизонтальная карусель 7 карточек, цитата и H2 на месте. Стрелки на `sm+`, автопрокрутка, пауза на hover/focus.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` инструменты 2×2:** блок «Редактирование фото с ИИ» — `grid-cols-2`, не 3 колонки на lg. Чипы галереи снова `flex-wrap`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` порядок чипов:** по показам Вебмастера на каталожных листингах тех же тегов, 14–20.08.2026. `/generaciya-foto/*` в query-analytics пустые. «Аниме» — последнее. Тот же порядок у карусели тем (`GENERACIYA_FOTO_SCENARIO_ROUTES`).
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` чипы:** на хабе нет чипа «Сделать фото ИИ». На детях чип хаба со стрелкой назад остаётся.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` галерея intro:** «Генерируй фото с ИИ на основе бесплатного онлайн каталога промтов».
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` галерея:** H2 «Идеи для фото ИИ», не «ИИ-фото от наших пользователей». FAQ-якорь `#primery` с той же подписью.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` инструменты:** 4 карточки (без «Описание изображения» и «Типаж по Кибби»). Ссылка «Все ИИ-инструменты» снята.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` инструменты:** H2 «Редактирование фото с ИИ», lead «ИИ инструменты для работы с фотографией».
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` бесплатные сервисы:** шаблоны `prompt` в Generate Dock на русском, без EN-инструкций.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` HowTo CTA:** кнопка в блоке шагов — «Создать фото», не «Создать свою фотосессию».
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` HowTo шаг 02:** «Выберите промт» / «Выберите промт из каталога или напишите свой», не «фотосессию».
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` FAQ короче:** сняты 6 вопросов (подписка, фотостудия онлайн, совместные фотосеты, нейрофотосессия с собой, вход, «фото не загружается») и нижняя кнопка после FAQ. Порядок заканчивается FAQ.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` FAQ-ссылки:** ответы — сегменты (`string | {href,label}`). В UI — `GeneraciyaFotoFaq`: `Link` на сервисы, `#якоря` через `scrollIntoView` (Next перехватывает hash и скроллит вверх). JSON-LD = `flattenGeneraciyaFotoFaqAnswer`. Якорь тем `#temy`. Нет Facee «Фотосессии» / «ИИ-редактор».
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` модели:** карточки как у остальных блоков (white/`rounded-2xl`/indigo «Выбрать»). Тексты = `GENERATION_MODEL_DISPLAY`. «Выбрать» = `requestModelSelection` → Generate Dock. Seedream 4.5 скрыт тем же `parseEnabledGenerationModels` (`id !== seedream-4.5`). Flux 2 Flex — только последняя генерация модели, без catalog fallback.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` отзывы:** сняты ссылки «Показать больше» / «Читать все отзывы о PromptShot» — якоря на `#otzyvy`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` темы:** CTA коллажа = название категории (`item.title`), не «Создать фото».
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` чипы хаб↔сценарии:** первый чип `getGeneraciyaFotoChipNavigation` → `/generaciya-foto` (zinc + иконка сетки / стрелка назад), темы остаются indigo. SSOT `generaciya-foto-chip-nav.ts`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` без примерки одежды:** из «Что ещё умеет PromptShot?» снята карточка «Примерка любой одежды онлайн».
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` без «идей»:** блок «Еще больше идей для фото» снят. Порядок: hero → темы → галерея → инструменты → HowTo → отзывы → что умеет → модели → FAQ → CTA.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` без 150+:** блок «Выбирайте из 150+ готовых ИИ‑фотосессий» снят. Порядок: hero → темы → галерея → инструменты → HowTo → отзывы → что умеет → модели → FAQ → CTA.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` бесплатные сервисы:** «Попробовать» = `seedBlankPrompt` шаблона из `GENERACIYA_FOTO_TOOLS.items[].prompt` → Generate Dock (`intent=text`, шторка `prompt`). Новый RPC/модалка не нужны. Гость — тот же auth-гейт dock.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` темы = 22 страницы генерации:** карусель «Подборки шаблонов по темам» = `GENERACIYA_FOTO_SCENARIO_ROUTES` (те же чипы). Коллаж — `resolve_route_cards` `sort=new` по dimension/tag страницы, без нового RPC.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` CTA стартера:** у вошедшего подпись всегда «Создать фото» (и в режиме «по фото»). Клик по фото-режиму по-прежнему открывает file picker.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` квота стартера:** карточки «по тексту / по фото» как были; чип анализа на фото-карточке — `minimal` (`N/10`, без полоски и «сегодня»). `/foto-v-promt` не трогали.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` соцстрока:** «Более N человек уже сгенерировали ИИ фото»; N = `COUNT(*)` completed image в `landing_generations` (ISR 3600), не Facee 2.5 млн.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` счётчики тем:** чип на коллаже = `total_count` из того же `resolve_route_cards` по dimension/tag страницы, не числа Facee. Спека `docs/26-08-sdelat-foto-ii.md`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` темы без «Смотреть все»:** единый блок компактнее (меньше паддинг и слайды), кнопка в галерею снята. Спека `docs/26-08-sdelat-foto-ii.md`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` темы = единый блок:** оболочка как у «ИИ-фото от наших пользователей» (`rounded-[1.75rem]`, indigo-градиент, тень). Заголовок и lead внутри, карусель на всю ширину блока. Спека `docs/26-08-sdelat-foto-ii.md`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` темы-карусель:** женские / мужские / парные / семейные — горизонтальный snap-ряд, `aspect-[4/5]`, стрелки, автопрокрутка с паузой на hover. Коллаж 6 фото + стеклянный оверлей. Картинки: `resolve_route_cards` + `getCardPhotosBySlugs`. Новых RPC нет. Спека `docs/26-08-sdelat-foto-ii.md`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` «Выбрать и повторить»:** CTA на hero-карусели открывает карточку из ленты в модалке `/p/` (ближайшая к центру). Стрелки листают те же 50 карточек карусели, как в листинге. Спека `docs/26-08-sdelat-foto-ii.md`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` hero-карусель:** над стартером «По описанию / По фото» — лента 50 новых карточек (`sort=new`), автопрокрутка справа налево, плитки как в листинге, пауза на hover. Галерея ниже по-прежнему 16. Спека `docs/26-08-sdelat-foto-ii.md`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` без hero «Создать фото бесплатно»:** под intro остаётся только «Посмотреть лучшее». Запуск генерации — в стартере `#generaciya-foto-starter-cta`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` тексты = facee.ru:** стартер без своих H2/eyebrow; соцстрока с 📸️; инструменты «Попробовать»; отзывы с кавычками Facee + «Показать больше»; FAQ — формулировки главной Facee (`Facee`→`PromptShot`, без Telegram/Т-Банка/буста). Спека `docs/26-08-sdelat-foto-ii.md`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` стартер по семантике:** eyebrow `ИИ генератор фото`, H2 `Сгенерировать фото ИИ`, lead «Создайте фото по описанию или по своему снимку». Ключ Title «сделать фото ии» в стартере не повторяем. Спека `docs/26-08-sdelat-foto-ii.md`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` тексты Facee донесены:** в темах — примеры с главной facee.ru; в 150+ — названия фотосессий; отзывы все 7; FAQ — ответы Facee (`Facee`→`PromptShot`, без Telegram/приложения/буста); CTA галереи «Больше промптов для фото». Спека `docs/26-08-sdelat-foto-ii.md`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` без трендов Facee:** снят блок «Последние ИИ-тренды». После тем сразу галерея.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` без цифр Facee:** сняты карточки «2.5 млн довольных клиентов» / «60 млн фото создано». Соцстрока hero не трогали.
>
> Последнее обновление: 2026-08-26 (**chrome CTA «Создать фото»:** таббар и desktop FAB `GenerateListingDockHost` — «Создать фото» / гость «Войти и создать фото». Busy/кредиты без изменений.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` без `#seo-opisanie`:** keyword-body ~4500 снят. Title/short и Facee-блоки остаются. Порядок: hero/starter → темы → галерея → инструменты → HowTo → отзывы → что умеет → модели → FAQ → CTA. Спека `docs/26-08-sdelat-foto-ii.md`.
>
> Последнее обновление: 2026-08-26 (**`/generaciya-foto` Facee-блоки + ключ «сделать фото ии»:** Title `Сделать фото ИИ онлайн`, H1 и секции — формулировки главной facee.ru (`Facee`→`PromptShot`). SSOT `generaciya-foto-seo-copy.ts`. Новых RPC нет. Спека `docs/26-08-sdelat-foto-ii.md`.
>
> Последнее обновление: 2026-08-26 (**OAuth one card viewer:** пин не помог — Next 15 после `pushState('/p/slug')` монтирует `@modal` / hard `/p/` без listing-handler, плюс snap prepend prev при `scrollTop=0` когда пин снимали по «slug совпал». SSOT owner — `client-card-overlay.ts` до pushState; `@modal` = `InterceptedCardModalGate`. Snap: сначала только current, соседи после центра, индекс и DOM совпадают. «Повторить» биндит slug, не live overlay соседа. Спека `docs/26-08-auth-return-card.md`.
>
> Последнее обновление: 2026-08-26 (**OAuth card pin SSOT:** `ps_ov=card:<slug>` — пин, не таймер. Restorer `pinAuthReturnCard` до `openCard`. Пока пин жив: snap не рендерит соседей в первый кадр, `goToNeighbor` / snap-commit / `open(other)` не меняют slug (кроме revert на пин). Соседи появляются после того как current === pin; пин снимается когда snap centered/`snapArmed` или desktop. Спека `docs/26-08-auth-return-card.md`.
>
> Последнее обновление: 2026-08-26 (**OAuth mobile prev-card + listing hydration:** restorer `useLayoutEffect` → `openCard` → `lockListingScrollForModal` писал inline overflow на `#listing-scroll-root` до hydrate PageLayout. Lock-стили только после `markListingScrollRootHydrated`. Snap не коммитит соседа, пока overlay только что открыт / `ps_auth` pending; соседи листинга читаются sync из nav storage, не после useEffect — иначе prepend prev даёт карточку «перед выбранной». Спека `docs/26-08-auth-return-card.md`.
>
> Последнее обновление: 2026-08-26 (**desktop→mobile width-only snap pin:** сжатие окна Chrome меняет ширину, высота та же. Inner scroller уже `display:block` + JS-height, CSS `snap-y` на скрытом feed клеит `scrollTop=0` — соседний hero без chrome (модальный X на mobile `max-md:hidden`). Usable = stage `md:hidden`, не inner. Crossing `max-width: 767px` всегда recenter + pin 12 frames, snap/overflow только после `snapArmed`. Спека `docs/26-08-auth-return-card.md`.
>
> Последнее обновление: 2026-08-26 (**desktop→mobile card snap recenter:** overlay slug — SSOT. Immersive snap-feed `md:hidden` на desktop `display:none` / `clientHeight=0`, но жесты были включены. Ресайз в mobile сбрасывает `scrollTop=0` → commit соседнего hero без контролов. Жесты только при `max-width: 767px` и usable viewport; hidden→visible recenters текущий slug и игнорирует layout-scroll. iOS chrome во время настоящего свайпа по-прежнему rebase progress. Спека `docs/26-08-auth-return-card.md`.
>
> Последнее обновление: 2026-08-26 (**OAuth listing pin against top:** close показывает верный Y, через 1–2 с `html { scroll-behavior: smooth }` + Next popstate/`scrollRestoration=auto` уводят наверх. Overlay close: manual restore, pin instant scroll, отбивать только snap в 0 (~2.2 с). Выход из `/p/slug` не `scrollCatalogToTop`. Спека `docs/26-08-auth-return-card.md`.
>
> Последнее обновление: 2026-08-26 (**OAuth listing fill-then-pin:** после `location.replace` в листинге только SSR-страница. `ps_sy=4342` нельзя писать сразу — clamp к подвалу, close снова restore и режет load-more, потом прыжок вверх. Пока карточка открыта: fill до `maxScroll >= Y`, применить один раз. Close = unlock, если уже на Y. Спека `docs/26-08-auth-return-card.md`.
>
> Последнее обновление: 2026-08-26 (**OAuth listing scroll Y:** экран возврата = path + overlay + `ps_sy`. Пока карточка открыта window часто 0 — Y берём из `SCROLL_KEY` (lock при open), не из текущего scroll. Destination `path?ps_auth=1&ps_ov=card:<slug>&ps_sy=<y>`. Restorer пишет Y, потом открывает карточку; `openCard` не затирает сохранённый Y нулём. Спека `docs/26-08-auth-return-card.md`.
>
> Последнее обновление: 2026-08-26 (**OAuth PKCE replay is success if session exists:** `@supabase/ssr` `createBrowserClient` включает `detectSessionInUrl` и сам делает первый `POST /token`. Второй `exchangeCodeForSession` на `/auth/callback` даёт `PKCE code verifier not found` — это не логаут. `finishOAuthCodeExchange` идемпотентен: живая сессия → `path?ps_auth=1&ps_ov=card:<slug>`, не `/?auth_error=`. Return path peek до успеха, consume после. Спека `docs/26-08-auth-return-card.md`.
>
...
>
> Последнее обновление: 2026-08-25 (**pairs hub chips + Direct H1:** `/promty-dlya-foto-par` как ДР — featured-чипы детей под H1 из `promty-dlya-foto-par-cluster.ts` (портрет, реалистичное, ч/б, студия, романтика, цветы, улица, интерьер). Соседи «Люди» в preGrid не показываем. При `yclid` / `yandex+cpc` клиент меняет H1 и `document.title` на заголовок объявления: каталог+дети+sitelink `s-parnem`/`s-muzhem`/`vlyublennykh` → «Промты для фото пары с ИИ»; `/generaciya-foto/pary` → «Сделайте парное фото с ИИ по вашим фото». `generateMetadata` / `og:title` не трогаем. SSOT кампаний — `yandex-two-cluster-launch.ts`. Спека `docs/25-08-yandex-pairs-cluster.md`.
>
> Последнее обновление: 2026-08-25 (**Seedream 4.5 via OpenRouter:** тот же пикер `seedream-4.5` / 10 кредитов / SQL `215` `enabled:false`. Worker `openrouter-seedream.ts`: sync `POST /api/v1/images` model `bytedance-seed/seedream-4.5`, ответ `b64_json`. Env `OPENROUTER_BASE_URL={GEMINI_PROXY_ORIGIN}/u/openrouter.ai` + `OPENROUTER_API_KEY`. Пустой base — `config_error`, без fallback на `openrouter.ai`. Референсы — signed Storage URL 15 мин (не `/u/`). 1K → 2K. Resume-poll Replicate нет: Image API синхронный. Gemini→Grok fallback не трогаем. Спека `docs/25-08-seedream-45-openrouter.md`.
>
...
>
> Последнее обновление: 2026-08-24 (**camera orbit azimuth ±180:** влево/вправо clamp 180°, шаг кнопки 30° (6 кликов = сзади). Высота по-прежнему ±60. Reveal-текст на 90/180 — профиль / спина, не «левая щека». Спека `docs/24-08-camera-orbit.md`.
>
...
>
> Последнее обновление: 2026-08-24 (**camera orbit UI:** оверлей ракурса использует тот же `GenerationResultActionRail`. Клик оси += 30° (повтор копит, yaw ±180, pitch ±60). Ghost по центру на подложке (`Камера слева 30°, высота как была`). Одна CTA `Снять кадр · N`. «Выйти» / X возвращают на исходник сцены. Спека `docs/24-08-camera-orbit.md`.
>
...
>
> Последнее обновление: 2026-09-01 (**prompt remix saves only:** CTA `Сохранить` пишет новый промпт и закрывает шторку. `POST /api/generate` не вызывается из remix. С кадра — rail «Создать фото» (local edit). Без кадра — обычный footer generate. Prefs API деградирует в defaults, если нет `landing_generation_preferences`.)
>
...
>
> Последнее обновление: 2026-08-20 (**birthday sitemap + L3 301:** тогда sitemap включал L3; с 2026-08-27 L3 снят, 301 на audience L2. `/sobytiya/1-sentyabrya` по-прежнему по SSOT + FTS. `DEN_ROZHDENIYA_PERMANENT_REDIRECTS` — audience-first L2 и L3→L2 wildcards в `next.config.ts`.)
>
...
>
> Последнее обновление: 2026-08-19 (**birthday search pagination:** search-backed листинги больше не обрываются на SSR-срезе. Первая страница — `LISTING_SEARCH_PAGE_SIZE` (48) с peek `limit+1`; `GET /api/listing?q=` отдаёт `has_more`. С 2026-08-27 hybrid материализует 500; `/api/search` остаётся на окне 100.)
>
> Последнее обновление: 2026-08-19 (**birthday anti-cannibal copy:** title/description/H2 хаба больше не перечисляют «девушке / детям / с тортом» и do-глаголы. Каждый L2 держит свой модификатор; combo L3 copy в `seo-content.ts` остаётся, но URL L3 с 2026-08-27 301 на audience L2. Генерация — `/generaciya-foto/na-den-rozhdeniya`. H1 хаба не менялся.)
>
...
>
> Последнее обновление: 2026-08-19 (**birthday listing = search query:** сетка хаба и детей `/sobytiya/den-rozhdeniya*` — поиск, не AND-каталог. С 2026-08-27 SSOT-запрос один: `день рождения`; L2 добавляет `audience_tag` / `object_tag`. Hybrid как `/search`, окно listing 500. SSR и `InfiniteGrid` пагинируют `GET /api/listing?q=` (+ фильтры L2). Index/noindex — по хитам первой страницы. Sitemap — хаб + L2.)
>
...
>
> Последнее обновление: 2026-08-16 (**listing explorer chrome:** `/[...slug]`, `/trends`, `/search` — тот же блок, что «Готовые промты для ИИ-фотосессии» на главной: SEO-текст (H1 + intro) внутри рамки, поле поиска, фильтры листинга вместо Wordstat-чипов, затем masonry. Крошки остаются над блоком; HowTo/FAQ/SEO — `SeoPageSections` под ним, тот же `GF_BLOCK` что на `/`. `CatalogWithFilters` больше не `dynamic()`.)
>
...
- **Баннер «Фото в промт»** снят со всех листингов и карточек. Legacy-цели **`foto_v_promt_banner_click`** / **`foto_v_promt_banner_click_card`** больше не вызываются. ТЗ — **`docs/requirements/04-06-foto-v-promt-mini-banner.md`**.
...
/ii-fotosessiya → Гибридный хаб кластеров «ИИ фотосессия по фото» и «промты для ИИ фотосессии». Title `ИИ фотосессия по фото онлайн — готовые промты`, H1 `ИИ фотосессия по фото`. Каркас: hero с прямой загрузкой + карусель серий → коллажи L2 → SSR-промты на русском → HowTo → галерея серий → тарифы `#tarify` → FAQ. Источник контента — только опубликованные completed photoshoot jobs / многофайловый web UGC; L2 дополнительно требует SEO-тег сценария. Copy SSOT `promty-dlya-ii-fotosessii-seo-copy.ts`, выборка `photoshoot-listing.ts` + `promty-dlya-ii-fotosessii-page-data.ts`, роуты `promty-dlya-ii-fotosessii-cluster.ts`. L2 `/ii-fotosessiya/{slug}` сохраняют сервисные H1 и добавляют промты в Title/H2. Sitemap хаб 0.9, дети от 8 релевантных серий 0.85. 301 `/promty-dlya-ii-fotosessii` + `/:slug` и `/promty-dlya-fotosessii` → соответствующие новые URL, не на главную. Спека `docs/done/31-08-ii-fotosessiya.md`.
/generaciya-foto         → SEO/product page «сделать фото ии»; Title `Сделать фото ИИ онлайн`, видимый H1 и блоки — формулировки facee.ru (`Facee`→`PromptShot`); copy SSOT **`generaciya-foto-seo-copy.ts`**; SSR hero = breadcrumb + H1 + соцстрока + intro + CTA + `GeneraciyaFotoStarter` (карточки «По описанию» / «По фото», pill-CTA). «По описанию» пишет `seed.intent=text` и открывает шторку `prompt`. «По фото» = ephemeral analyze → `seedBlankPrompt(..., intent=photo_prompt)` **без** записи в `landing_user_photos`. Desktop FAB прячется, пока `#generaciya-foto-starter-cta` в зоне видимости (`heroCtaInView`); mobile — таб «Создать фото». Photo→prompt канон — `/foto-v-promt`. Last-completed hydrate только при `intent=resume` и без `lastDockResultDismissed` (`shouldHydrateLastDockResult`). `photo_prompt` compose не восстанавливает library photos (`shouldAttachLibraryPhotos`). Все surfaces выровнены единым wide-container contract. Ниже — searchable masonry examples (H2 Facee «ИИ-фото от наших пользователей», CTA «Больше промптов для фото»), enabled model showcase и Facee HowTo. Initial examples сериализуются как compact `GenerationExampleCard`; клик по плитке открывает модалку карточки без hover-кнопки «Повторить». Model CTA пишет requested model в `GenerateDockContext`, поэтому lazy mount не теряет selection. Models анимируют один image layer с deferred frame switching и contextual `alt`. Explorer ищет через `/api/search`, быстрые фильтры используют `/api/listing` и остаются crawlable ссылками на L1 без JS. Reference photo optional для blank text-to-image. Canonical и JSON-LD WebApplication+BreadcrumbList+HowTo+FAQPage+ItemList всегда (без fake offers/ratings); `robots` index + `max-image-preview:large`; sitemap **0.9** и inbound SEO-links всегда. Sitemap endpoints fail-soft. Отдельные `/generaciya-foto/*` не создаются
...
| `/api/generation-config` | Конфиг генерации; `modality=image` (default) или `video`. Image: модели из `models` (Nano Banana / PRO / 2 / Lite + **Grok Imagine** `grok-imagine-image-2.0` + **Seedream 4.5 / 5.0 Pro** + **Flux 2 Flex**, OpenRouter-модели по 10 кредитов), `cameraOrbitEnabled` из `camera_orbit_enabled` **или** allowlist, `cameraOrbitModel` из `camera_orbit_model` (дефолт Grok, id должен быть в `models`), `photoshootEnabled` / `photoshootModel` / `photoshootCreditCost` из `photoshoot_enabled` / `photoshoot_model` (SQL `224`/`228`, дефолт Gemini 3 Pro) + `PHOTOSHOOT_CREDIT_COST` (15 кр). Video: модели из `video_models` (Grok 1.5 + Veo Omni Flash + Veo 3.1 Lite + **Seedance 2.5** `seedance-2.5`, 24 кр/сек, SQL `220` `enabled:false`), `defaults.model` = `DEFAULT_VIDEO_MODEL` (`veo-3.1-lite-generate-preview`) через `resolveVideoModelId`, 9:16/16:9, 4/6/8/10 сек (Lite без 10), 720p, `enabled` из `video_animate_enabled` **или** allowlist (`azarov.maxim@gmail.com`). Оба modality отдают `listingVideoRepeatEnabled` из `listing_video_repeat_chain` (SQL `234`), `composeExampleMatchEnabled` из `compose_example_match_enabled` (SQL `240`) и `publishReward` {enabled, photo, video, photoshoot, dailyCap} из `landing_generation_config` (SQL `231`). SSOT `lib/generation/image-options.ts`. |
| `/api/generation-preferences` | GET/PUT (auth): SSOT последних compose-настроек JWT user — image model / aspect / size, video model / aspect / duration, owned photo IDs. PUT клампит stale model и отбрасывает чужие/удалённые photo IDs, не валит весь снимок. Клиент не пишет image model до первой completed photo-генерации и не подставляет `default_model` Flash на первом джобе (`compose-image-model.ts`). LWW `localStorage`. SQL `173` + `209`. |
...
| `/api/user-generation-photos` | GET (auth): библиотека inline-фото текущего JWT user, newest-first, с signed preview URL и кэшем `audienceTag` для пикера примеров |
| `/api/user-generation-photos/[id]` | DELETE (auth): удаление принадлежащего пользователю фото из private Storage и библиотеки |
| `/api/generate/animate-scenario` | POST (auth): по плитке «Видео» (шторка «Модель видео») или кнопке «Оживить» на готовом фото — `gemini-2.5-flash` смотрит owned image (parent generation XOR upload path), возвращает короткий RU-сценарий (1–2 предложения) **с этого кадра** (frame 0, без lead-in). Исходный image-промпт не подмешивается, а stash-ится и возвращается при выборе фото-режима. Не списывает кредиты. SSOT `lib/video-animate-scenario.ts` + `lib/compose-modality-prompt.ts`. Proxy как у remix. |
| `/api/generate` | Auth enqueue: image — **0–10** owned upload-фото без `editInstruction` (0 = text-only) или local edit (`parentGenerationId` + `editInstruction`). Camera orbit: `editKind=camera_orbit` + `cameraPose` + parent image; сервер резолвит `scene_root_id`, сериализует instruction в `prompt_text` (бриф карточки не шлёт), I2I с корня, модель/кредиты = `camera_orbit_model` (дефолт Grok); флаг `camera_orbit_enabled`. Photoshoot: `editKind=photoshoot` + **parent image (не лист) XOR ровно одно owned upload**; planner всегда `temperature=2` (клиентский `plannerTemperature` игнорируется, в `edit_instruction` пишется `planner_temperature=2.00`); `image_size` всегда **2K**; 1 job / 15 кр (`PHOTOSHOOT_CREDIT_COST`), модель `photoshoot_model`; I2I fail → один хоп Flux 2 Flex `safety_tolerance=5` (`photoshoot_fallback_model`, SQL `233`); worker vision planner → 2×2; флаг `photoshoot_enabled`. Listing video Repeat: `pipeline.kind=listing_video_repeat` + ровно одно owned фото + image prompt (look); modality форсится в image; баланс ≥ image+video; `pipeline_spec` на image job; `create_ugc=false`; worker после complete enqueue video I2V (`parent_generation_id`, motion, idempotency `listing-video-repeat:{imageId}`). Флаг `listing_video_repeat_chain` (SQL `234`). Compose-плитка шлёт upload из «Ваши фото»; rail после кадра — parent. `photoshoot_busy` по parent или тому же `input_photo_paths`. `GET /api/generations/:id/camera-scene` — плёнка. Image-модель должна быть из enabled `models` (неизвестный id → 400, не `models[0]`; у orbit id только из `camera_orbit_model`, иначе 503). Grok Imagine: 10 кредитов, 4K clamp в 2K. Video — `modality=video`, ровно одно фото **или** owned completed image parent, без edit; 4/6/8/10 сек / 720p / модель из `video_models` (дефолт Veo 3.1 Lite, Lite max 8 сек; Seedance 2.5 — 24 кр/сек); кредиты = `calculateVideoCreditCost`; `create_ugc=false`. Fingerprint включает modality, duration и для orbit — pose/root. RPC `landing_enqueue_generation` (миграции **189**, **200**, **201**, **202**, **204**, **207**, **212**); ответ `202 { id, status: pending }` |
...
| `/api/generations` | Auth-список `landing_generations` через RPC `landing_list_my_generations` (SQL `239`; fallback `.or(requester, legacy billed)`). `limit`≤50, `offset`, peek `limit+1` → `{ generations, hasMore, nextOffset }` с `resultUrl` + listing `resultThumbUrl` / `photoshootTileThumbUrls`; batch `prompt_cards` → `cardId/cardSlug/isPublished`; без `executed_model`. private no-store. **DELETE** `{ ids: uuid[] }` (≤50) — bulk hard-delete owned rows; result object удаляется только если не используется `prompt_card_media` |
| `/api/generations/[id]` | GET: статус/результат генерации. Для image с `pipeline_spec.kind=listing_video_repeat` отдаёт `pipelineSpec` + `followupGenerationId` / `followupStatus`; если image completed и child нет — тот же followup enqueue, что у worker. **DELETE**: hard-delete owned row; `409 generation_in_use`, если result нужен active child; object сохраняется при ссылке из `prompt_card_media` (UGC `prompt_cards` не удаляется) |
...
| `/api/generations/[id]/publish` | POST (auth owner): ensure draft → общий SEO publish service → `is_published=true`; идемпотентный success для уже опубликованной карточки. После publish — RPC `landing_grant_publish_reward` (флаг `publish_reward_enabled`). Ответ `{ reward }` |
...
| `/api/my-cards/[slug]/visibility` | PATCH (auth): `{ published: boolean }` — владелец переключает видимость; при `published: true` — LLM/regex тегирование (`landing/src/lib/seo-tags-classify.ts`), затем `revalidatePath`. Тот же publish-reward RPC, если есть owned `landing_generations.ugc_card_id` |
| `/api/me` | Текущий пользователь + credits + живой грант `{ offer: { percent, expiresAt, showNudge, … } \| null }` + `publishRewardRemainingToday`. Для logged-in не-гостя сначала `landing_ensure_low_balance_upgrade` (идемпотентно), затем `landing_live_pricing_offer`. Гость — полная цена, `offer: null`. Шапка читает баланс; `PricingCards` рисует зачёркнутый каталог; `LowBalanceUpgradeNudge` показывает карточку на маунте |
...
| `/api/payments/unpaid-banner` | GET (auth): latest unpaid ЮKassa ∪ Robokassa за 24 ч + live offer. Без reconcile. Плашка `UnpaidCheckoutBanner`. |
| `/api/payments/yookassa/unpaid-banner` | GET: alias на `/api/payments/unpaid-banner`. |
...
- **Точка входа карточки `/p`:** treatment — `CardPageClient` → `onInternalGenerate`. Гость: `beginGuestCardRepeatAuth` + AuthModal, карточка остаётся. Authed: `GenerateDock.seedFromCard` (`catalogCardRepeatSeed`: `cardId` + промт + https-превью в плитку «Выбрать пример») + close overlay / `router.replace(last listing)` (не `history.back`). Hover «Повторить» на листинге — тот же `seedFromCard`, не STV. Control — LexyGPT.
- **Глобальный generate-dock:** `GenerateDockContext` + `GenerateListingDockHost` в `PageLayout` (allowlist листингов, treatment). Composer = `CardInlineGeneratePanel chrome=dock`. Expand surfaces (`prompt|photos|model`) растягивают плашку. После `phase=done` `resultUrl` — фон **внутри** пластины (clip + radius) через `GenerationResultBackdrop`: follow-up gen держит предыдущий кадр в CSS-pixelate, по completed — preload + clip-path reveal нового (reduced-motion: blur + opacity). `needsCredits` / `credits===0` → indigo→violet CTA на FAB, mobile tab и footer compose («Купить кредиты для создания фото») → pricing overlay, без error-баннера. Mobile tab open → fullscreen `inset-0 z-[122]` поверх tab/nav; desktop: старт gen → collapse + FAB progress; live job `done` → reopen plate (mobile и desktop) и закрыть шторку. Последний completed кадр — `lastDockResult` в контексте: tab/FAB на fotosessii восстанавливают его (не шторку «Ваши фото»). Hydrate `GET /api/generations` для blank `resume` и `photoshoot`, если контекст пуст и пользователь не сбросил result («Повторить» / delete / result X → `lastDockResultDismissed`). HowTo / hero остаются upload-first. Tab / FAB: `listingGenerateIdleIntent` → `seedPhotoshoot` / `seedBlankPrompt(photo_prompt)` / `focusBlank`. Guest FAB/tab открывают dock (CTA «Войдите» на enqueue). Guest card «Повторить» — AuthModal, не dock (`resolveCardRepeatAction`). Live overlay `card` не перезаписывается `generate-dock`. Sidebar indigo CTA на `/generate` убран.
- **Inline photo library:** `CardInlineGeneratePanel` при открытии параллельно читает `GET /api/user-generation-photos` и `GET /api/generation-preferences`, показывает persistent-карусель квадратных preview по `created_at DESC` и восстанавливает **все** доступные выбранные photo IDs (не одно первое). Если prefs-строки ещё нет — самое свежее фото; явный `[]` остаётся text-only; если все сохранённые id удалены — снова самое свежее. Hydrate merge не дропает in-session upload и identity-ephemeral (stale GET). Карусель доступна поверх result backdrop. Плитка «Добавить» всегда первая; новые файлы и SEO-selfie после auth → `POST /api/upload-generation-photo` `saveToLibrary=true` на флаге `libraryUploading` (не `phase=uploading`); **0–10** фото. Гость: только in-memory data URL, persist при появлении сессии. `photo_prompt` в библиотеку не пишется. Удаление — `DELETE /api/user-generation-photos/[id]`. Overlay шторки/desktop scrim закрывается по `pointerdown`.
...
- **OAuth resume dock:** пока plate открыт, live overlay `generate-dock:<intent>` (cookie + `ps_ov`). Restorer открывает модалку с тем же инструментом — IdP hop не обязан сохранить sessionStorage. Guest seed пишет `promptshot:pending-generate-dock` без data:/blob: preview и sidecar `promptshot:pending-compose-example` (`cardId` + промт). Consume только на listing (`shouldRestorePendingGenerateDock`), не на `/auth/callback`. `seedForAuthReturnDock` мержит sidecar, если pending съели или selfie persist был без `cardId`. `GenerateDockProvider` consume-once, если restorer overlay не было.
...
- **Финансы (live P&L):** страница `/admin/finance` фильтр
  ...
  Директ — `admin_finance_ads_lines` только если импорт `direct_api`
  (cron/кнопка Reports API, кабинет без НДС); загруженный CSV Директа
  в этом режиме не участвует.
  Операционная маржа = gross − касса − УСН 6% − AI COGS (`netIncomeRub`).
  После Директа = операционка − кабинет × **1,22**.
  Переключатель «Тянуть данные из CSV» (`csv=1`) включает monthly override
  через `admin_finance_replace_import` (ЮKassa/GCP/Direct). Загрузка файла
  включает CSV. Файлы остаются в БД и не удаляются при «Нет».
...
- **Flow после `POST /api/generate`:** API через один SECURITY DEFINER RPC проверяет `Idempotency-Key`, списывает кредиты и создаёт `pending`; отдельный `web-generation-worker` poll-ит очередь, атомарно claim-ит batch (image и video отдельно), вызывает image-провайдер (Gemini `generateContent`, Grok Imagine `images/generations|edits` или OpenRouter Seedream/Flux `POST /api/v1/images`) или video-адаптер (Grok xAI / Omni Flash Interactions / Veo 3.1 Lite LRO / OpenRouter Seedance 2.5) и сохраняет результат. HTTP self-fetch отсутствует.
- **Image provider route:** `job.model` → Grok (`grok-imagine-image*`), OpenRouter (`seedream-*` / `flux-*`) или Gemini. Grok: `web-generation-worker/src/xai-image.ts`, те же `XAI_API_KEY` / `XAI_BASE_URL` что у video, sync `POST /v1/images/generations` (text-only) или `/v1/images/edits` (до 3 data-URI), `quality=medium`, `response_format=b64_json`, resolution `1k|2k` (4K → 2K). Промпт — `assembleGrokImage*` без Gemini-тегов. Пустой `XAI_BASE_URL` → `config_error`, без fallback на `api.x.ai`. OpenRouter: `web-generation-worker/src/openrouter-seedream.ts`, `OPENROUTER_BASE_URL={GEMINI_PROXY_ORIGIN}/u/openrouter.ai` + `OPENROUTER_API_KEY`. Sync `POST /api/v1/images`, `n=1`, `output_format=png`. Vendor: `seedream-4.5` → `bytedance-seed/seedream-4.5` (2K\|4K), `seedream-5.0-pro` → `bytedance-seed/seedream-5-0-pro` (1K\|2K), `flux-2-flex` → `black-forest-labs/flux.2-flex` (без `resolution`). Ответ — `data[].b64_json`; URL качать только если host `openrouter.ai` через `/u/`. Референсы — signed Storage URL 15 мин (не `/u/`). Пустой `OPENROUTER_BASE_URL` / нет `/u/` → `config_error`, без fallback на `openrouter.ai`. Image API синхронный: resume-poll нет, heartbeat lease во время POST (до 180 с). Circuit как у Grok image (20/8/50%/60с). SQL `218` включает 5.0 Pro и Flux.

- **Image fallback:** любая ошибка Gemini на image-job (включая `IMAGE_OTHER` и safety) на не-Grok/не-Seedream/не-Flux модели → тот же attempt один раз зовёт Grok; если Grok тоже упал, недоступен или circuit open — тот же attempt зовёт Seedream 5.0 Pro. Primary Grok fail → Seedream. Primary Seedream/Flux — без следующего хопа. Skip только `shutdown`. `fallback_used` + `executed_model` persist; retry после Grok-хопа сразу в Grok, после Seedream-хопа сразу в OpenRouter по `executed_model`. Кредиты не пересчитываются. Circuit Grok и Seedream по отдельности (20/8/50%/60с). Kill-switch: `image_fallback_model` (Grok) и `image_fallback_secondary_model` (Seedream) пустые или `enabled:false` у модели.
- **Photoshoot fallback:** `edit_kind=photoshoot` после fail I2I (вкл. `safety_block`) — тот же attempt один раз `photoshoot_fallback_model` (`flux-2-flex`, SQL `233`) с `safety_tolerance=5`. Планер тот же. Circuit Seedream хоп не глушит. Primary Flux — без следующего vendor. Обычные фото без этого хопа. Кредиты не пересчитываются. Выкл: пустой ключ / `enabled:false` у Flux.
- **Video fallback:** любая ошибка выбранной **не-Grok** video-модели после вызова вендора (Veo Lite / Omni / Seedance), включая `safety_block`, timeout, 5xx и `config_error` ключа — тот же attempt один раз зовёт `grok-imagine-video-1.5`. Primary Grok — без следующего vendor. Skip: `shutdown`, ошибки до submit (input/parent/crop/storage), `result_upload_error`. На хопе persist `fallback_used` + `executed_model` и **сброс** `provider_operation_id` (иначе resume поллит Veo/OpenRouter id на xAI). В xAI body всегда `grok-imagine-video-1.5`, не `job.model`. Кредиты не пересчитываются (Lite 15 остаётся 15). Circuit `grokVideoCircuit` 20/8/50%/60с. Kill-switch: `video_fallback_model` пустой / false или `enabled:false` у Grok в `video_models`. SQL `221`. Спека `docs/25-08-video-grok-fallback.md`.
- **Claim / backpressure:** миграция `sql/170_landing_generation_queue.sql` + `189` (`p_modality`); image и video claim-ятся отдельно. Image: concurrency 10, global 50, per-user 3, lease 180s. Video: concurrency 2, global 8, per-user 1, lease 600s. Video пишет `provider_operation_id` до poll (resume без повторной оплаты). Seedream/OpenRouter — sync POST, id пишется после успеха только для аудита.
...
- **Текст в Gemini (без `vibe_id`):** если есть входные фото — worker склеивает **`prompt_text`** + wardrobe rules (`assembleLandingCardFinalPrompt`). Default **`wardrobe_policy=replace`**: идентичность с фото, **гардероб по тексту промпта**. Чип «Оставить одежду» + флаг `preserve_outfit_enabled` пишет **`keep`**: lock одежды/обуви/аксессуаров с input photo, секция `Clothing` гасится (`wardrobe-policy.ts`). Grok/Seedream — те же policy. Фотосессия / orbit / vibe / local edit этот контракт не трогают. Если фото нет (`sourceType=text_only`) — **`assembleTextToImageFinalPrompt`** без identity-preservation. Pure source of truth: `landing/src/lib/image-generation-prompt.ts`, он же компилируется в worker. SQL `235`.
...
- **Video provider route:** `job.model` → Grok (`grok-imagine-*`), Veo Lite (`veo-3.1-lite*`), Seedance (`seedance-*`) или Gemini Omni. Перед submit worker cover-crop'аит исходное фото под `job.aspect_ratio` (9:16/16:9, `video-source-frame.ts`) — Grok и Seedance получают signed URL обрезанного JPEG, Veo/Omni — inline base64. Seedance: `web-generation-worker/src/openrouter-seedance.ts`, те же `OPENROUTER_API_KEY` / `OPENROUTER_BASE_URL={GEMINI_PROXY_ORIGIN}/u/openrouter.ai` что у Seedream image (не `GEMINI_PROXY_BASE_URL`, пустой / без `/u/` → `config_error`, без fallback на `openrouter.ai`). Async `POST /api/v1/videos` model `bytedance/seedance-2.5`, `frame_images` first_frame, `generate_audio=true`, 720p, 4/6/8/10 сек; persist `provider_operation_id` до poll; download только `GET /api/v1/videos/{id}/content` через `/u/` (не чужой host из `unsigned_urls`). Промпт — `assembleSeedanceVideoMotionPrompt`. Отдельный `seedanceVideoCircuit` 20/8/50%/60с. Цена — 24 кредита/сек (`calculateVideoCreditCost`), SQL `220` `enabled:false`. Grok: `web-generation-worker/src/xai-video.ts`, `XAI_BASE_URL={GEMINI_PROXY_ORIGIN}/u/api.x.ai` (тот же DO-vhost, не `GEMINI_PROXY_BASE_URL`, без fallback на `api.x.ai`), signed URL кадра 15 мин, `assembleGrokVideoMotionPrompt` без Gemini-тегов. Veo Lite: `web-generation-worker/src/veo-video.ts`, `POST /v1beta/models/{id}:predictLongRunning` через `GEMINI_PROXY_BASE_URL`, poll operation, `personGeneration=allow_adult`, длительность 4/6/8, 720p, `assembleVeoVideoMotionPrompt`. Omni: SSOT `video-interaction.ts` → `buildVideoInteractionRequest`. Официальный unary: `background=false`, `store=false`, `stream=false`. `video_config` только `{ task: "image_to_video" }`; `aspect_ratio` и `duration` (`4s`–`10s`) в `response_format`. Gemini на этом task принимает **ровно одно** изображение — второе даёт `invalid_request`. Текст Omni — `assembleVideoMotionPrompt`: `[# Sources @Image1]` + identity lock. Submit логирует `provider`, `proxyHost` (xAI) или `httpStatus`/`imageBytes` (Gemini/Veo). Источник кадра: `video_input_resolved`. `user_photos` = аплоад; копия из «Использовать» поднимается в generation result как единственный кадр. `generation_result` = готовое фото.
...
- **Страница:** `/generations` — «Мои генерации» в меню пользователя. Первая страница — RSC (`getSupabaseUserFromServerCookies` + `listUserGenerations`); клиент не дублирует mount-fetch. Дальше sentinel и poll идут в `GET /api/generations`. Список — RPC `landing_list_my_generations` (два indexed UNION: requester history + legacy `requester IS NULL AND credits_spent > 0 AND user_id = billing`; индекс `idx_landing_gen_legacy_billing_history`, SQL `239`). UI — тот же chrome, что каталог (`ListingGrid` + `GenerationHistoryCard` 3:4): сетка рисует listing thumbs (`next/image`, `PhotoshootListingGrid`) и in-view `ListingCardVideo`; dock / download / share берут полные `resultUrl` / `photoshootTileUrls`. Статусы `pending` / `processing` / `failed` как плейсхолдер в том же фрейме. Completed **фото и видео** кликабельны одинаково: `seedCompletedResult` (`intent=result`) открывает generate-dock с тем же result chrome, что после генерации (кадр + rail действий). «Оживить» только на фото. Карточка `/p/[slug]` с клика не открывается. На готовом фото, если `video_animate_enabled`, внизу плитки в одну линию glass-кнопки «Оживить» (`seedAnimate`) и «Опубликовать» (`+N✦`). Видео и фотосессия — только «Опубликовать». Фотосессия без `photoshoot_tile_paths` всё равно отдаёт 4 sidecar URL от листа (`resolvePhotoshootUserFacingResult`); иначе карточка зависала на «Готово…». Overflow-меню: выбрать (bulk delete), поделиться, скачать, скопировать промпт, использовать (в библиотеку для генерации), опубликовать, удалить — без «Оживить». Ответ списка не кешируется. `/generate` (`GenerateBlankShell`) по-прежнему грузит историю клиентом.
...
- **Feature flag:** продукт — БД. `landing_feature_rollouts.payment_robokassa` (`enabled` + `rollout_bps`, 5000 = 50%) и sticky `landing_user_feature_assignments`. Canary — `landing_generation_config.robokassa_canary_emails` (плюс legacy env `ROBOKASSA_CANARY_EMAILS`). `PAYMENT_PROVIDER=robokassa` остаётся аварийным 100%. Resolver: живой unpaid 24ч пинит PSP → canary → env force → bucket. Нет строки / `enabled=false` / ошибка чтения → YooKassa. `POST /api/payments/create` и оба create-route зовут один `resolvePaymentProvider`; клиент `provider` не шлёт. Спека `docs/01-09-robokassa-traffic-split.md`, SQL `236`.
- **Robokassa UX:** официальный `robokassa_iframe.js` загружается один раз по требованию; `Robokassa.Render` получает подписанный сервером payload и `Settings={Mode:'modal'}`. `PaymentMethods` намеренно не ограничен: iframe показывает все способы, доступные магазину Robokassa. Backend передаёт `Email` из подтверждённой auth-сессии, поэтому адрес для чека уже заполнен; невалидный/отсутствующий email не отправляется. Платёжная форма открывается поверх PromptShot, `window.location.assign` не вызывается. Официальный handler на `action=redirect` уводит **top window**; PromptShot ставит capture-guard раньше него: `Merchant/State` остаётся в iframe (иначе «Вернуться в магазин» теряет parent), same-origin Success/Fail закрывает overlay, чужой ACS/3DS host пропускаем. `/payment/robokassa/*` дополнительно закрывает overlay, если SuccessURL всё же открылся внутри iframe.
...
- **Paid + SEO acquisition:** visitor/session, first-touch cookie `promptshot_utm` (21 день), immutable visitor→shared-user link, pre-auth funnel и payment snapshot — `sql/196_landing_acquisition_attribution.sql` + апгрейд unpaid→paid в `sql/211_seo_traffic_attribution_upgrade.sql`. Реклама: UTM/`yclid` как в URL. Органика: `document.referrer` → `yandex_seo`/`google_seo`/`bing_seo` + `organic` + первый pathname; пустой referrer → `direct`/`none`; чужой host → `referral`. Same-origin, OAuth и **PSP checkout** (`yoomoney.ru`, `yookassa.ru`, `money.yandex.ru`, `robokassa.ru` — SSOT `payment-provider-hosts.ts`, тот же список что у `isSafeYooKassaConfirmationUrl`) не классифицируем: `identity`, куку не пишем. Возврат `?payment=<uuid>` и `/payment/*` не посадочные (`isExcludedLandingLocation`). Снимок оплаты: paid checkout по-прежнему перекрывает unpaid user и hole-fill same-tier paid; same-tier unpaid берёт bag пользователя целиком (не merge `utm_content` с реферера шлюза). Старый клиент, приславший `referral` + `yoomoney.ru`, на create отбрасывается в empty и падает на user. Платный канал перетирает SEO на visitor, `landing_users` и ledger; второй paid и любой unpaid поверх paid — нет. Finance Директа по-прежнему `yandex|ya` + `cpc`; `yandex_seo` туда не попадает. Shared guest owner не person identity; ads не меняет текущий `netIncomeRub`. Спеки: `docs/19-08-traffic-source-attribution.md`, `docs/19-08-yandex-direct-acquisition.md`, `docs/24-08-seo-traffic-attribution.md`.
- **Robokassa confirmation:** `GET|POST /api/payments/robokassa/result` разбирает form-urlencoded ResultURL, проверяет signature через Пароль №2, `InvId`, `Shp_payment_id` и сумму, затем вызывает `landing_fulfill_robokassa_payment`; ответ строго `OK{InvId}`. SuccessURL не подтверждает оплату. `RobokassaPaymentStatus` в root layout опрашивает только owned local status с backoff, закрывает iframe после `succeeded` и обновляет баланс после callback.
...
- **Return UX:** клиент polling с backoff ~2→5→10 с до ~20 попыток (~2–3 мин) если есть `?payment=`. Иначе `YooKassaReturnStatus` один раз (и на `visibilitychange`) бьёт `POST /api/payments/yookassa/open-reconcile`. Сервер reconcile’ит и локальный `canceled`, если ещё нет `credited_at`. Не вешать сверку на `GET /api/me`. Если latest unpaid ЮKassa ∪ Robokassa без `credited_at` — `UnpaidCheckoutBanner` над навбаром 24 ч (`GET /api/payments/unpaid-banner`). Бар скрыт при `?payment=` и на `/payment/*`. После закрытия iframe Robokassa плашка снова видна, если платёж ещё unpaid. С 15-й минуты и живом гранте 25% — copy про 60 мин + countdown до `expires_at` письма. «Продолжить» делает новый create: ЮKassa → hosted URL, Robokassa → iframe. Крестик пишет `payment_id` в localStorage. INSERT Robokassa ставит те же abandon due, что ЮKassa.
...
AuthModal / SidebarAccountPanel → OAuthSignInButtons → signInWithOAuth(redirectTo: /auth/callback)
  ...
  → client page: finishOAuthCodeExchange → path?ps_auth=1&ps_ov=card:<slug>&ps_sy=<y>
...
Почему не server route: дублирующий `GET /auth/callback` делал второй `POST /token` (`user_agent=node`) → `404 flow_state_not_found`, а ответ дубля отдавал `?auth_error=` без session cookies. В браузере первый обмен пишет cookies в document.

Почему replay PKCE — не ошибка логина: `createBrowserClient` в `@supabase/ssr` 0.9 **принудительно** ставит `detectSessionInUrl: true` (опция `auth.detectSessionInUrl` перетирается). Layout/`AuthProvider` создаёт singleton на `/auth/callback?code=` → клиент сам меняет code на сессию и стирает verifier. Страница callback делает второй `exchangeCodeForSession` → `PKCE code verifier not found`. Класс: **идемпотентный finish**. Живая `getSession`/`getUser` (или `invalid flow state`) = успех и destination с `ps_ov`. Без сессии — `auth_error`. Strict Mode remount делит один in-flight по `code`. `resolveOAuthNextPath` **peek** cookie/sessionStorage, consume только после успеха — иначе первый mount съедает listing path и remount уходит на `/`.
...
Fallback: если `code` пришёл на произвольную страницу (не `/auth/callback`), `AuthProvider` делает client `exchangeCodeForSession` и при наличии сохранённого return path уводит туда с тем же маркером. На `/auth/callback` `AuthProvider` **не** обменивает code (навигацию пишет `finishOAuthCodeExchange`; неявный `/token` всё равно может сделать singleton).

- Хелпер: `landing/src/lib/auth-oauth.ts` + `auth-return-path.ts` + `auth-return-screen.ts` + `auth-finish-oauth.ts` + `auth-session-hydrate.ts`. `redirectTo` без `?next=` — иначе GoTrue часто срывает query и падает на `SITE_URL=/`. Overlay карточки едет в `?ps_ov=card:<slug>`, позиция листинга — в `?ps_sy=<y>` (пока карточка открыта window часто 0, Y берём из `card_modal_scroll_pos`). После reload restorer **не** ставит Y, пока `maxScroll < Y`: fill грузит страницы под карточкой, apply один раз, close без повторного restore. `/p/slug` без live overlay всё равно card. SITE_URL `/?code=` всегда `location.replace` с маркером. При старте OAuth: Yandex → `force_confirm=yes`, Google → `prompt=select_account`.
...
`GenerationModelsShowcase` получает для каждой enabled-модели последнюю завершённую генерацию, связанную с опубликованной `prompt_cards` через `landing_generations.ugc_card_id`. Приватные и неопубликованные пользовательские результаты не попадают в SSR. Запросы по моделям выполняются параллельно, изображение строится из `result_storage_bucket/result_storage_path`. Если опубликованной генерации нет, карточка может взять каталоговый fallback — **кроме Flux 2 Flex**: только последний кадр этой модели, иначе плейсхолдер. Seedream 4.5 не приходит из `parseEnabledGenerationModels`, даже если в JSON `enabled:true`.
...
**Каталог vs ИИ-фотосессия:** `promty-dlya-foto-*` (кроме хабов пар, девушки и мужчины), `/stil/`, `/sobytiya/`, object L1 (`/s-mashinoy` и соседи) — один кадр: скопировать промт или Repeat. Кластер «промты для ИИ фотосессии …» принадлежит `/ii-fotosessiya/*`, **кроме** `/promty-dlya-foto-par` (пары), explorer-H2 на `/promty-dlya-foto-devushki` («промты для ИИ фотосессии женские») и explorer-H2 на `/promty-dlya-foto-muzhchiny` («промты для ИИ фотосессии мужские»). `/ii-fotosessiya/zhenskie` и `/ii-fotosessiya/muzhskie` остаются владельцами серии луков. Чип «ИИ-фотосессия» в `popularLinks` на прочих L1 — сосед (серия луков), не замена Title/H1 хаба.
...
**Кластер «девушки» (каталог):** единственный индексируемый владелец — хаб `/promty-dlya-foto-devushki`. Title/H1: `Промты для фото девушки` (+ `7000+` в Title); intro: «женских фото»; explorer H2: «промты для ИИ фотосессии женские». HowTo нет. `7000+` — округлённый SEO-лейбл (`GIRLS_PROMPT_COUNT_LABEL`); точный объём — `ListingPromptCountBadge`. Карусель: `audience_tag=devushka` `sort=new` (`girlsHubHeroFetchParams` игнорирует `?style=` / `?object=`). Чипы — query-фильтры (`GIRLS_HUB_FILTER_CHIPS`). Все L2 `/promty-dlya-foto-devushki/*` кроме `den-rozhdeniya` → HTTP 301 на хаб (`girlsChildRedirectPath` / `listingCatalogHubChildRedirectPath`). ДР остаётся в `DEN_ROZHDENIYA_PERMANENT_REDIRECTS`. Idle FAB — `Создать фото девушки`, чип примера `Девушки`. Page chrome — тот же `GeneraciyaFotoHeroPage`, что у пар; развилка в page одна (`resolveListingCatalogHubL1`).

**Кластер «мужчины» (каталог):** единственный индексируемый владелец — хаб `/promty-dlya-foto-muzhchiny`. Title: `Промты для фото мужчины и мужских фото — 1300+ идей`; H1: `Промты для фото мужчины`; intro: «мужского фото»; explorer H2: `Промты для ИИ фотосессии мужские`. `1300+` — округлённый SEO-лейбл (`MEN_PROMPT_COUNT_LABEL`); точный объём — `ListingPromptCountBadge`. Карусель: `audience_tag=muzhchina` `sort=new` (`menHubHeroFetchParams` игнорирует `?style=` / `?object=`). Чипы — query-фильтры (`MEN_HUB_FILTER_CHIPS`: машина, деловое, портрет, студия, форма, ч/б, костюм). Все L2 `/promty-dlya-foto-muzhchiny/*` кроме `den-rozhdeniya` → HTTP 301 на хаб (`menChildRedirectPath` / `listingCatalogHubChildRedirectPath`). ДР остаётся в `DEN_ROZHDENIYA_PERMANENT_REDIRECTS`. Idle FAB — `Создать фото мужчины`, чип примера `Мужчины`. Page chrome — тот же `GeneraciyaFotoHeroPage`; развилка в page одна (`resolveListingCatalogHubL1`). `/ii-fotosessiya/muzhskie` не трогаем.

**Кластер «пары» (каталог):** единственный индексируемый владелец — хаб `/promty-dlya-foto-par`. Title: `Промты для ИИ фотосессии пары — 800+ готовых промтов на русском`; H1: `Промты для ИИ фотосессии пары`; description — та же лемма + «нейросети»; intro — «парной фотосессии» + скопировать промт / два фото. Explorer H2: `Промт для нейросети для фотосессии пары`.

Хаб **владеет и подкластером «с парнем»**: семь подтверждённых форм запроса дают 3007 показов и 37 кликов за 28 дней (CTR ≈ 1,23 %). Exact-match больше не в сниппете: только один FAQ и чип-фильтр. Intro, HowTo, Title, description и H2 не повторяют фразу. `/promty-dlya-foto-s-parnem` больше не индексируется: точный HTTP 301 на хаб через `pairsChildRedirectPath` / middleware, из sitemap и меню исключён; homepage-чип ведёт сразу на `/promty-dlya-foto-par?audience=s_parnem`. Листинг, фильтры и masonry сознательно не меняли — поведенческие метрики хаба уже хорошие, низкий был только CTR в выдаче.

`800+` — округлённый SEO-лейбл (`PAIRS_PROMPT_COUNT_LABEL`); точный объём каталога показывает `ListingPromptCountBadge` из `totalCount`. Хаб исключён из общего запрета photoshoot-ключей в каталоге. Все дочерние `/promty-dlya-foto-par/*` сразу консолидируются точным HTTP 301 на хаб через `middleware.ts`; из sitemap, верхней навигации и нижних combo-групп они исключены. `pairsChildRedirectPath` — общий предикат middleware и sitemap. Стили/сюжеты и «кто» на хабе — query-фильтры (`PAIRS_HUB_FILTER_CHIPS` → `?audience=s_parnem` / `?style=cherno_beloe` / `?object=v_mashine` / `?occasion=novyy_god`), canonical остаётся хаб. Чипы и фильтры — один `flex-wrap`: сюжетные и audience-чипы (с парнем / влюблённые), затем `ListingDesktopFilters inline` как последние чипы того же списка (`display: contents`, без отдельной колонки и без горизонтального скролла). Чипа «С мужем» на хабе нет. `/promty-dlya-foto-s-parnem` — точный HTTP 301 на хаб (`PAIRS_SITELINK_REDIRECT_PATHS`). Независимые L1 `/promty-dlya-foto-s-muzhem` и `/promty-dlya-foto-vlyublennykh` остаются 200; чипы хаба на sitelink-URL больше не ведут. Хаб пар использует тот же page chrome, что главная: `GeneraciyaFotoHeroPage` (open hero + `GF_PAGE_STACK`), не слот внутри listing-карточки. Hero: H1 / intro / счётчик / `GeneraciyaFotoHeroCarousel` из `audience_tag=para` `sort=new` (`pairsHubHeroFetchParams`), без overlay-CTA и без текущего query-фильтра. `CatalogExplorer` ниже с `hideHeading` и H2 `explorerTitle` над поиском («промт для нейросети для фотосессии пары»), как gallery-heading на главной. Листинг: `teaserLoadMore` + кнопка `Больше промтов для пар` (fade как на `/generaciya-foto/devushki`, отступ над generate dock), затем «Показать ещё». HowTo / FAQ — `SeoPageSections` (как на главной); `seoTextBlocks` и mid-page generate-CTA на хабе нет. `popularLinks` хаба пустые. Idle FAB / tab — `Создать фото пары`: `intent=text` + шторка «Выбрать пример» с чипом `Пары` (`audience_tag=para`). Независимые L1 `s-parnem` / `s-muzhem` / `vlyublennykh` не входят в дочерний префикс и сохраняются. Рекламный H1/title — клиентский `resolveAdLandingTitle` (не `utm_term`): каталог + бывшие дочерние URL + sitelink-хабы → «Промты для фото пары с ИИ» до серверного редиректа; generate — `/generaciya-foto/pary` через `AdLandingHeading`. SEO `generateMetadata` / `og:title` не подменяются рекламным H1.
...
OAuth completion: `/auth/callback` → `finishOAuthCodeExchange` → `path?ps_auth=1&ps_ov=`. `AuthReturnScreenRestorer` пишет `pinAuthReturnCard` + `beginClientCardOverlay` и открывает **один** `ClientCardModal`. Next 15 sync `pushState('/p/slug')` в App Router не должен дать второй `CardPageClient` (`InterceptedCardModalGate` + secondary viewer `return null`). Snap не коммитит соседа с `scrollTop=0`. «Повторить» биндит slug в `ps_ov`. Legacy `code` на SITE_URL всегда `location.replace` с тем же маркером, не `replaceState`.
...
- **Карточка:** `generateMetadata` → `buildCardMetaTitle(title_ru, slug)` (без `Visual Hook:`, суффикс ` | PromptShot`, лимит 80, `· shortId` при шаблоне/обрезке); OpenGraph, Twitter, `noindex` для thin/secondary карточек
...
| `listing_query_embeddings` | 768-d Gemini **query** vectors только для birthday listing SSOT `q` (SQL `223`). Не общий кэш `/api/search` |
...
| `get_listing_query_embedding` / `upsert_listing_query_embedding` | Read/write birthday listing query vectors (SQL `223`, service_role) |
| `landing_list_my_generations` | Owner history for `/generations` и `GET /api/generations`: requester UNION legacy billing, slim columns, service_role (SQL `239`) |
...
**Пагинация листинга (`InfiniteGrid` + `GET /api/listing`):** для теговых URL — **`LISTING_SSR_INITIAL_LIMIT` (10)** и **`LISTING_INFINITE_PAGE_SIZE` (24)**. Для search-backed (слабые L2 кластера «день рождения») — **`LISTING_SEARCH_PAGE_SIZE` (48)** на SSR и в клиенте; запрос материализуется hybrid-ом до 500 id на 1 час (`LISTING_HYBRID_MATERIALIZE_LIMIT`, SQL `222` `least(500, p_limit)`), ключ кэша включает фильтры, страницы режутся из кэша, peek `limit+1`, флаг **`has_more`**. Хаб и теговые L2 пагинируются как категорийные URL. Публичный `/api/search` остаётся на text window 100 и embed timeout 800 мс. Listing cache-fill сначала читает `listing_query_embeddings`; live embed **8000 мс** только на miss. Visual ANN — отдельный RPC. В теговом ответе API есть **`ranked_batch_size`** (число строк из RPC до `expandCardGroups`) и **`sort`**. Следующий **`offset`** увеличивается на это значение, а не на `cards.length`: иначе split-группы раздувают массив, OFFSET в SQL перескакивает через «недопоказанные» ранги и сетка листинга визуально «перемешивается». Условие «есть ещё страницы»: `hasMoreRankedPages` = `offset + ranked_batch_size < total_count`. Автодогрузка — **`useListingSentinelLoadMore`**: IO + drain после settle, если сенсор всё ещё в `LISTING_SENTINEL_ROOT_MARGIN_PX`. Смена **`sort`** или query-фильтра → remount `InfiniteGrid` (key включает `mergedRpcParams` + sort), **`offset=0`**, **`resetListingScroll()`**. Empty state при `sort=new` и `total_count=0`: «Пока нет новых». Риск дубликатов/пропусков при живом **`popularity_score`** + OFFSET — как с `view_count`; follow-up: keyset pagination. Повтор `card.id` между страницами (sibling с прошлой порции + ranked row) снимается в **`appendUniqueCardPage`** (`landing/src/lib/listing-cards.ts`) — offset по ranked не откатываем, чтобы не зациклить infinite scroll; drain продолжает, если порция целиком схлопнулась в дубли. `StableListingMasonry` последовательно продолжает детерминированные 2/3/4 lanes: добавление порции не меняет placement prefix и не создаёт page-boundary gaps. Публичный рендер — **одна masonry-плитка на `card.id`**, без `GroupedCard`.

**Плитки листинга (`ListingPhotoTile`):** то же визуальное правило, что главная и блок генерации — aspect первого фото, клик → модалка. Главная/короткие блоки используют CSS columns; infinite-листинги — `StableListingMasonry` с абсолютными lanes в container-query units. Hover-chrome / `listing-grid-clamp` на этих лентах не используются (`ListingGrid` 3:4 остаётся у `/generations` и `/analyses`). **`priority`** (`LISTING_LCP_PRIORITY_GRID_ITEMS` = 12) — `next/image priority` / `fetchPriority` у первых плиток. Pagination skeleton — **`ListingMasonrySkeleton`**. Legacy **`PromptCard` / `GroupedCard`** + `ListingCardLoadingShell` сохранены в коде, но публичные ленты их не монтируют. **Alt:** дефолт `buildCardImageAlt` (`Промт для фото: …`). Catalog hub (`[...slug]` + `headingAltSlots`) подставляет тот же H1/H2+хук, что hero; `aria-label` ссылки совпадает с `img alt` (Inspect часто читает label, не alt).
...
│   ├── promty-dlya-foto-par-cluster.ts ← SSOT хаба пар, featured-чипы, ad-path
│   ├── promty-dlya-foto-devushki-cluster.ts ← SSOT хаба девушки, чипы, 301, hero params
│   ├── promty-dlya-foto-muzhchiny-cluster.ts ← SSOT хаба мужчины, чипы, 301, hero params
...
│   ├── hero-carousel-alt.ts    ← H1/H2 + хук для img alt catalog hub / hero
...
| `OPENROUTER_API_KEY` | Worker only: Seedream / Flux Image API. Не на лендинге, не в git |
| `OPENROUTER_BASE_URL` | Worker: `{GEMINI_PROXY_ORIGIN}/u/openrouter.ai`. Пустой или без `/u/` — OpenRouter-job `config_error`, без прямого `openrouter.ai` |
...
| `SEARCH_VISUAL_TIMEOUT_MS` | Query embed timeout для `/api/search`, default 800. Listing SSOT использует 8000 мс в коде, не этот env |

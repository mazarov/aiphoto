...
> Последнее обновление: 2026-08-25 (**Seedream 4.5 via Replicate:** пикер `seedream-4.5` за 10 кредитов, SQL `215` с `enabled:false` до смоука прокси. Worker `replicate-seedream.ts`: async POST без `Prefer: wait` → persist `provider_operation_id` → poll, `Cancel-After: 180`. Все вызовы через `REPLICATE_BASE_URL={GEMINI_PROXY_ORIGIN}/u/api.replicate.com`, download через `/u/replicate.delivery`. Пустой base — `config_error`, без fallback на `api.replicate.com`. 1K clamp → 2K. Gemini→Grok fallback не трогаем. Спека `docs/25-08-seedream-45-replicate.md`.
>
...
- **Flow после `POST /api/generate`:** API через один SECURITY DEFINER RPC проверяет `Idempotency-Key`, списывает кредиты и создаёт `pending`; отдельный `web-generation-worker` poll-ит очередь, атомарно claim-ит batch (image и video отдельно), вызывает image-провайдер (Gemini `generateContent`, Grok Imagine `images/generations|edits` или Seedream 4.5 / Replicate `predictions`) или video-адаптер (Grok xAI / Omni Flash Interactions / Veo 3.1 Lite LRO) и сохраняет результат. HTTP self-fetch отсутствует.
- **Image provider route:** `job.model` → Grok (`grok-imagine-image*`), Seedream (`seedream-*`) или Gemini. Grok: `web-generation-worker/src/xai-image.ts`, те же `XAI_API_KEY` / `XAI_BASE_URL` что у video, sync `POST /v1/images/generations` (text-only) или `/v1/images/edits` (до 3 data-URI), `quality=medium`, `response_format=b64_json`, resolution `1k|2k` (4K → 2K). Промпт — `assembleGrokImage*` без Gemini-тегов. Пустой `XAI_BASE_URL` → `config_error`, без fallback на `api.x.ai`. Seedream: `web-generation-worker/src/replicate-seedream.ts`, `REPLICATE_BASE_URL={GEMINI_PROXY_ORIGIN}/u/api.replicate.com` + `REPLICATE_API_TOKEN`. Async `POST /v1/models/bytedance/seedream-4.5/predictions` (**без** `Prefer: wait`) + `Cancel-After: 180` → persist `provider_operation_id` → poll `GET /v1/predictions/{id}` через rewrite `urls.get`. Референсы — signed Storage URL 15 мин (не `/u/`, Replicate качает сам). Output качается только с `replicate.delivery` / sibling через `{GEMINI_PROXY_ORIGIN}/u/<host>`. 1K → 2K (`seedream_size_clamped`), 4K разрешён, `sequential_image_generation=disabled`, `max_images=1`. Пустой `REPLICATE_BASE_URL` / нет `/u/` → `config_error`, без fallback на `api.replicate.com`. Resume после рестарта воркера: только GET, второй POST запрещён. Circuit как у Grok image (20/8/50%/60с). SQL `215` вставляет пункт с `enabled:false`; флип после allowlist nginx (`api.replicate.com`, `replicate.delivery`).
...
- **Claim / backpressure:** миграция `sql/170_landing_generation_queue.sql` + `189` (`p_modality`); image и video claim-ятся отдельно. Image: concurrency 10, global 50, per-user 3, lease 180s. Video: concurrency 2, global 8, per-user 1, lease 600s. Video и Seedream submit пишут `provider_operation_id` до poll (resume без повторной оплаты).
...
| `REPLICATE_API_TOKEN` | Worker only: Seedream 4.5. Не на лендинге, не в git |
| `REPLICATE_BASE_URL` | Worker: `{GEMINI_PROXY_ORIGIN}/u/api.replicate.com`. Пустой или без `/u/` — Seedream-job `config_error`, без прямого `api.replicate.com` |

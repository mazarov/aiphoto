...
| MobileTabBar | `components/MobileTabBar.tsx` | 5-вкладочный таб-бар (max-lg): **Новое** (`/new`) / Каталог / **Фото в промт** (center accent pill + подпись → `/foto-v-promt`) / Поиск / Профиль. LexyGPT из таббара убран. Регистрирует `registerMobileSearchOpen`, рендерит `ListingMobileSearchSheet` и `MobileProfileSheet`. |

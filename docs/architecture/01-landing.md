...
| `/api/generate` | Auth enqueue: image — **0–10** owned upload-фото без `editInstruction` (0 = text-only) или local edit (`parentGenerationId` + `editInstruction`). Video — `modality=video`, ровно одно фото **или** owned completed image parent, без edit; 4/6/8/10 сек / 720p / модель из `video_models` (дефолт Grok 1.5, Lite max 8 сек); кредиты = база + доплата за длительность; `create_ugc=false`. Fingerprint включает modality и duration. RPC `landing_enqueue_generation` (миграции **189**, **200**, **201**, **202**); ответ `202 { id, status: pending }` |

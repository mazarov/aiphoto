...
- **Эффект:** generate с `creditsCharged=0`; `landing_generations.user_id` = JWT пользователя (не guest-owner); upload/poll только для своего `user_id`.

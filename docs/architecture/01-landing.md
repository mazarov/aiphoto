...
| `/api/generation-config` | Конфиг генерации; `modality=image` (default) или `video`. Image: модели из `models` (Nano Banana / PRO / 2 / Lite + **Grok Imagine** `grok-imagine-image-2.0`, 5 кредитов). Video: модели из `video_models` (Grok 1.5 + Veo Omni Flash + Veo 3.1 Lite), `defaults.model` через `resolveVideoModelId`, 9:16/16:9, 4/6/8/10 сек (Lite без 10), 720p, `enabled` из `video_animate_enabled` **или** allowlist (`azarov.maxim@gmail.com`). SSOT `lib/generation/image-options.ts` |
...
| `/api/prompt-remix` | POST (auth): принимает текущий editable `prompt + changeRequest` и optional owned completed `parentGenerationId`, возвращает только переписанный prompt без создания generation. Нормализованное эхо источника → `422 unchanged_prompt` (история не пишется). Успех best-effort сохраняется в `analyze_history` (`kind=remix`, `change_request`, итоговый `prompt`, `user_id` = shared `dbUserId`, без image). Логи `[prompt.remix]`: `gemini_request` (полный source + change), `gemini_response` (extracted prompt + Google candidates/usage/error). System instruction требует интегрировать изменение во все затронутые секции и удалить противоречия, а не дописывать финальную строку. Модель `GEMINI_PROMPT_REMIX_MODEL` (default `gemini-2.5-flash`), `temperature=0.3`, `thinkingBudget=0`, `maxOutputTokens=8192`; ответ с `MAX_TOKENS` не принимается как готовый. Proxy определяется через `photo_app_config.gemini_use_proxy` + `GEMINI_PROXY_BASE_URL` |
...
| `/api/generate` | Auth enqueue: image — **0–10** owned upload-фото без `editInstruction` (0 = text-only) или local edit (`parentGenerationId` + `editInstruction`). Image-модель должна быть из enabled `models` (неизвестный id → 400, не `models[0]`). Grok Imagine: 5 кредитов, 4K clamp в 2K. Video — `modality=video`, ровно одно фото **или** owned completed image parent, без edit; 4/6/8/10 сек / 720p / модель из `video_models` (дефолт Grok 1.5, Lite max 8 сек); кредиты = база + доплата за длительность; `create_ugc=false`. Fingerprint включает modality и duration. RPC `landing_enqueue_generation` (миграции **189**, **200**, **201**, **202**, **204**); ответ `202 { id, status: pending }` |
...
| `/api/imageprompt-proxy/extension/analyze` | Dev-only same-origin прокси к `imageprompt.tools/api/extension/analyze`. `getImagePromptAnalyzeUrl()` в `next dev` ведёт сюда (`/foto-v-promt`); prod — `/api/extension/analyze`. |
...
- **Analyze site flow:** `/foto-v-promt` вызывает `getImagePromptAnalyzeUrl()`:
  в prod — same-origin `POST /api/extension/analyze`, в `next dev` —
  `/api/imageprompt-proxy/extension/analyze` → `imageprompt.tools`
  (локальный Gemini proxy с dev-машины часто timeout).
  Виджет шлёт `image_url` или `image_base64` (`image-prompt-analyze-client.ts`)
  и **не** пишет файл в `landing_user_photos` для ephemeral analyze.
...
- **Image fallback:** eligible провайдерный сбой (429/5xx/timeout/empty, не safety/input/config) на не-Grok модели → тот же attempt один раз зовёт Grok. `fallback_used` + `requested_model` persist, следующие retry сразу в Grok. Кредиты не пересчитываются. In-process circuit: 50% ошибок из ≥8 за последние 20 / cooldown 60 с — фолбек skip, прямой выбор Grok жив. Kill-switch: `landing_generation_config.image_fallback_model` пустой или `enabled:false` у Grok.
...
- **Paid acquisition:** visitor/session, inbound UTM first-known non-direct, immutable visitor→shared-user link, pre-auth funnel и payment snapshot — `sql/196_landing_acquisition_attribution.sql`. Ручной CSV Директа (`kind=ads`), cohort CAC/ROAS/LTV и data quality — `sql/197_admin_finance_yandex_direct.sql`. Shared guest owner не person identity; ads не меняет текущий `netIncomeRub`. Спеки: `docs/19-08-traffic-source-attribution.md`, `docs/19-08-yandex-direct-acquisition.md`.
...
- **Конверсия в Метрику:** обе fulfillment-ветки шлют Measurement Protocol (`ea=purchase` + ecommerce, `ti=payment.id`, `tr=amount_rub`) на `mc.yandex.ru/collect`; lease в `yandex_conversion_claimed_at` не допускает параллельный дубль. Robokassa запускает отправку через Next `after()`, поэтому ResultURL сразу получает `OK{InvId}` после durable fulfillment. Ошибки Метрики оплату не роняют. Return/status poll дополнительно стреляет JS `purchase` + `dataLayer`.
- **YooKassa confirmation (три consumer’а):** (1) webhook, (2) return-poll `YooKassaReturnStatus`, (3) cron/admin stale sweep. Все пути делают `GET /v3/payments/{id}`, сверяют provider ID, metadata, RUB-сумму и статус. Обе provider-specific fulfill RPC блокируют ledger row и в одной транзакции начисляют сохранённые credits ровно один раз.

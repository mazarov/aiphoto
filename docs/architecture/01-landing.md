...
- **Flow после `POST /api/generate`:** API через один SECURITY DEFINER RPC проверяет `Idempotency-Key`, списывает кредиты и создаёт `pending`; отдельный `web-generation-worker` poll-ит очередь, атомарно claim-ит batch (image и video отдельно), вызывает image-провайдер (Gemini `generateContent`, Grok Imagine `images/generations|edits` или Seedream 4.5 / OpenRouter `POST /api/v1/images`) или video-адаптер (Grok xAI / Omni Flash Interactions / Veo 3.1 Lite LRO) и сохраняет результат. HTTP self-fetch отсутствует.
- **Image provider route:** `job.model` → Grok (`grok-imagine-image*`), Seedream (`seedream-*`) или Gemini. Grok: `web-generation-worker/src/xai-image.ts`, те же `XAI_API_KEY` / `XAI_BASE_URL` что у video, sync `POST /v1/images/generations` (text-only) или `/v1/images/edits` (до 3 data-URI), `quality=medium`, `response_format=b64_json`, resolution `1k|2k` (4K → 2K). Промпт — `assembleGrokImage*` без Gemini-тегов. Пустой `XAI_BASE_URL` → `config_error`, без fallback на `api.x.ai`. Seedream: `web-generation-worker/src/openrouter-seedream.ts`, `OPENROUTER_BASE_URL={GEMINI_PROXY_ORIGIN}/u/openrouter.ai` + `OPENROUTER_API_KEY`. Sync `POST /api/v1/images` model `bytedance-seed/seedream-4.5`, `n=1`, `output_format=png`, `resolution=2K|4K`. Ответ — `data[].b64_json`; URL качать только если host `openrouter.ai` через `/u/`. Референсы — signed Storage URL 15 мин (не `/u/`, OpenRouter качает сам). 1K → 2K (`seedream_size_clamped`), 4K разрешён. Пустой `OPENROUTER_BASE_URL` / нет `/u/` → `config_error`, без fallback на `openrouter.ai`. Image API синхронный: resume-poll нет, heartbeat lease во время POST (до 180 с). Circuit как у Grok image (20/8/50%/60с). SQL `215` вставляет пункт с `enabled:false`; флип после allowlist nginx (`openrouter.ai`) и `proxy_read_timeout` ≥ 180s.

- **Image fallback:** любая ошибка Gemini на image-job (включая `IMAGE_OTHER` и safety) на не-Grok/не-Seedream модели → тот же attempt один раз зовёт Grok; если Grok тоже упал, недоступен или circuit open — тот же attempt зовёт Seedream 4.5. Primary Grok fail → Seedream. Primary Seedream — без следующего хопа. Skip только `shutdown`. `fallback_used` + `executed_model` persist; retry после Grok-хопа сразу в Grok, после Seedream-хопа сразу в Seedream. Кредиты не пересчитываются. Circuit Grok и Seedream по отдельности (20/8/50%/60с). Kill-switch: `image_fallback_model` (Grok) и `image_fallback_secondary_model` (Seedream) пустые или `enabled:false` у модели.
...
| `OPENROUTER_API_KEY` | Worker only: Seedream 4.5. Не на лендинге, не в git |
| `OPENROUTER_BASE_URL` | Worker: `{GEMINI_PROXY_ORIGIN}/u/openrouter.ai`. Пустой или без `/u/` — Seedream-job `config_error`, без прямого `openrouter.ai` |

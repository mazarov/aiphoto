...
> Последнее обновление: 2026-08-14 (**photo-tab analyze:** вкладка «По фото» на `/generaciya-foto` вызывает тот же `getImagePromptAnalyzeUrl()`, что и `/foto-v-promt`. В `next dev` это `/api/imageprompt-proxy/extension/analyze` → `imageprompt.tools` (локальный Gemini proxy с localhost часто timeout 503); в prod — same-origin `/api/extension/analyze`.)
>
...
| `/api/imageprompt-proxy/extension/analyze` | Dev-only same-origin прокси к `imageprompt.tools/api/extension/analyze`. `getImagePromptAnalyzeUrl()` в `next dev` ведёт сюда (`/foto-v-promt` и вкладка «По фото» на `/generaciya-foto`); prod — `/api/extension/analyze`. |
...
- **Analyze site flow:** `/foto-v-promt` и вкладка «По фото» на `/generaciya-foto`

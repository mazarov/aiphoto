...
> Последнее обновление: 2026-08-07 (**inline image edit chain:** после результата `CardInlineGeneratePanel` заменяет повтор на CTA «Изменить картинку». Свободная правка проходит через существующий `extension/remix`, затем `POST /api/generate` создаёт новую job с `parentGenerationId`; worker использует completed result parent как единственный image input. Миграция `171` добавляет owner-validated lineage, исходные `prompt_cards/prompt_variants` не изменяются.)
>
...
- **Inline engine:** initial run: выбранные `storagePath[]` → `POST /api/generate` (`vibeId=null`, `cardId`) → `generationId` → poll. Edit run: `activePrompt + changeRequest` → `extension/remix` → новый prompt → `POST /api/generate { parentGenerationId }`; API и enqueue RPC проверяют owned completed parent, worker читает его объект из `web-generation-results`. Каждая итерация становится новым parent. Result menu переиспользует `GenerationCardMenu` без bulk-select: share/download/copy, `save-to-library`, `publish`, `DELETE`.
...
- **Резолв:** `landing/src/lib/resolve-db-user-id.ts` — `landing_users`/`imageprompt_users` по JWT id → иначе `google_sub` из OAuth identity → иначе email. Используется в `/api/me`, `ensureLandingUserForGeneration`, list/get generations, **`/api/my-prompt-cards` (`/generations`)**, ownership visibility, card SSR/API viewer, vibe save `author_user_id`.

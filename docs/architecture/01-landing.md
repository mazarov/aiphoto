...
>
> Последнее обновление: 2026-08-28 (**AI-фотосессия user-facing:** лист 2×2 не отдаём клиенту. `GET /api/generations` и `/:id` ставят `resultUrl` = первый тайл, `photoshootTileUrls` = 4 кадра (`resolvePhotoshootUserFacingResult`). Оверлей без «Скачать лист» и без CSS crop. История — сетка 4 кадров. UGC publish — **одна** `prompt_cards` + 4 `prompt_card_media` (тайлы, не лист, не `card_split`). Старый черновик с одним кадром дописывается на ensure/publish. save-to-library — тайл 1. Split fail валит job.
>

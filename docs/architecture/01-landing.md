...
- **Кнопка «Сгенерировать» → LexyGPT** (отдельные цели JS в Метрике): **`lexygpt_generate_tabbar`** (`MobileTabBar`); **`lexygpt_generate_promptcard`** (`LexyGptGenerateButton` на листинге / `/p/[slug]`); **`lexygpt_generate_photovprompt`** (результат и история на `/foto-v-promt`). Legacy **`lexygpt_generate_click`** + `placement` — deprecated.

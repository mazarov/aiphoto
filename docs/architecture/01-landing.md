...
- **Условия:** session email в allowlist; `STV_OPEN_GENERATE_DEBUG` по умолчанию on для allowlisted (в т.ч. prod), `=0`/`false` выключает. Без allowlisted сессии bypass выключен → 401 / обычные credits.

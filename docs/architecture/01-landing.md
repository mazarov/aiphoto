...
> Дополнение 2026-08-02 (**pricing-local-test-preview:** в development неавторизованный пользователь может открыть `/pricing?test=true`; без параметра действует общий email allowlist, а в production query-параметр не обходит проверку доступа.)
...
/pricing                → Приватный preview только для `azarov.maxim@gmail.com` (остальным 404; в development прямой preview без сессии разрешён только с `?test=true`): пакеты Проба, Старт, Про, Максимум; до ЮKassa кнопка показывает non-blocking статус «Оплата появится скоро»

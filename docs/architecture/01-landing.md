...
- `illustrations` (опционально, 0–4) — SEO-фото с `alt`/`caption` под Wordstat-запросы; `cardSlug` или `titleIncludes` для подбора кадра из кластера
...
| `illustrations` | 4 шт. | HF intro: «промт для фото с машиной»; FAQ #2 авто, #3 сирень, #4 номера — alt с точным вхождением; фото из каталога (`cardSlug` / `titleIncludes`) |

**Рендер иллюстраций:** только L1 (`route.level === 1`). Intro — под hero-текстом; FAQ — компактная фигура слева от вопроса. Резолв: `getCardPhotosBySlugs` → fallback `fetchRouteCards` + `titleIncludes`. Schema: отдельный `ImageObject` на каждую иллюстрацию.

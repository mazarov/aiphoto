...
- **Панель extension/embed — редактируемый промпт:** в карточке «Шаг 1» основной текст — **`<textarea id="stv-gen-prompt-body">`** (можно править и вставлять свой). В состояние синхронизируются **`mergedForSingleGeneration`** и все **`prompts[].prompt`** (в legacy passthrough тело одно); перед постановкой генераций значение забирается из поля ещё раз. Отдельный **`<details>`** показывает **`finalPromptForGeneration`** (как сервер склеит через **`assembleVibeFinalPrompt`**). Ввод — debounced **`persistState`** (~400ms). Повтор строки («Вновь») подхватывает текст из поля, если он менялся.

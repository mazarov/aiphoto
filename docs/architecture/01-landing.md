...
**Каталог vs ИИ-фотосессия:** `promty-dlya-foto-*` (кроме хаба пар), `/stil/`, `/sobytiya/`, object L1 (`/s-mashinoy` и соседи) — один кадр: скопировать промт или Repeat. Кластер «промты для ИИ фотосессии …» принадлежит `/ii-fotosessiya/*`, **кроме** `/promty-dlya-foto-par`: хаб пар владеет prompt-интентом «ИИ-фотосессия пары» и «парных фото». Чип «ИИ-фотосессия» в `popularLinks` — сосед (серия луков), не замена Title/H1 хаба.

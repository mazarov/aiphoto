...
/sobytiya/1-sentyabrya  → L1 кластера «Промты для фото» (не catch-all). Copy — **`seo-content.ts`** (`1_sentyabrya`). Explorer — `CatalogWithFilters`; стартовая masonry — **`search_cards_text("1 сентября")`**. Чипы над сеткой — события своего кластера; под сеткой — стили. На `/stil/*` L1 — чип «1 сентября» → этот URL.

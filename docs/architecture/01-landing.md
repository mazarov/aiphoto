...
/extension-stv          → Превью маркетингового лендинга расширения (спека `docs/extension-landing-pain-hope-solution.md`); `metadata.robots` noindex; CTA: env **`NEXT_PUBLIC_STV_CHROME_STORE_URL`** (иначе заглушка `#stv-chrome-store`). Блок **Reference** (`PainReferenceVsDraftMock`): **Add photo** → `POST /api/upload-generation-photo` → signed URL → `POST /api/vibe/extract` → `POST /api/vibe/expand`; промпт в колонке draft = `finalPromptForGeneration` (тот же pipeline, что в `stv-core.js`); требуется авторизация (cookie).
```
...
- **Точка входа:** кнопка **Steal This Vibe** на `/p/[slug]` (`GenerateButton`: один градиент как primary STV, иконка sparkles без вложенного «лого»-тайла), при `NEXT_PUBLIC_ENABLE_TRY_THIS_LOOK` или `debugOpen`.
- **UI:** `GenerationModal` — выезжающая панель справа + iframe **`/embed/stv?...`**; внутри тот же пайплайн, что в расширении (extract → expand → generate, см. § Vibe Pipeline ниже). Ранний отдельный React-флоу (`GenerationModal` + `PhotoUploader` + прямой `POST /api/generate` без vibe) **снят**.
- **Параметры iframe:** `cardId`, `sourceImageUrl` (герой карточки) — см. `GenerationContext`, `CardPageClient`.
...
- **Extract:** `POST /api/vibe/extract` — body: **`imageUrl`** + опционально **`extractTemperature`**. **`imageUrl`** — публичный HTTP(S); панель extension/embed может передать URL с сайта или **свежий signed URL** для пути в `web-generation-uploads` после загрузки референса с ПК (`GET /api/upload-generation-photo/signed-url`). Vision → JSON **9 строковых полей** по **`LEGACY_EXTRACT_PROMPT_2C23CE94`**: **`scene`** — место/действие, без волос/лица/телосложения модели (нейтральный субъект); **`pose`** — геометрия тела; **`camera`**, **`composition`** (passthrough expand), остальные как раньше. Строки в БД без **`pose`**: при expand **`coerceLegacyVibeStylePayload`** подставляет **`LEGACY_POSE_MISSING_BACKFILL`**. Extension: пресеты температуры. Провайдеры: **`vibe_extract_llm`**, модели (`sql/150_*.sql`). Insert **`vibes.style`**, **`prompt_chain` = `legacy_2c23`**, **`legacyPromptChain: true`**.
...
- **Панель extension/embed — редактируемый промпт:** **`<textarea id="stv-gen-prompt-body">`** и чекбокс **«Свой промпт»** — в **секции «Настройки»** (шаг 2), всегда видны после входа; боковая карточка со стилем (JSON) — только превью extract + кнопка pipeline-spec. В состояние синхронизируются **`mergedForSingleGeneration`** и все **`prompts[].prompt`**; перед постановкой генераций значение забирается из поля ещё раз. **`<details>`** с **`finalPromptForGeneration`** — когда не режим своего промпта. Ввод — debounced **`persistState`** (~400ms). Чекбокс **`customPromptMode`**: при включении поле очищается, **`expand` / assemble** при «Сгенерировать» не вызываются; **`runExtract`** выполняется; при снятии галочки при **`vibeId`** — **`runExpand`**. Grooming отключены в своём промпте. Персист **`stv_state_v2.customPromptMode`**.

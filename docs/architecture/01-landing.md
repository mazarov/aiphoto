...
> Последнее обновление: 2026-03-22 (STV expand: **scene literal** — без LLM merge; extract 8 полей **legacy_2c23**; extension до 4 фото — `docs/23-03-stv-multi-user-photos-ui.md`)
...
- **Expand:** `POST /api/vibe/expand` — legacy **`style`** из body и/или строки vibe; **`vibeId`** + владелец + **`prompt_chain` = `legacy_2c23`** (иначе **404** / **409** как раньше). **Без text LLM:** **`mergedPrompt`** = **`style.scene`** (trim); пустой `scene` → **400** **`missing_scene`**. **`prompts`**: три элемента с **одинаковым** текстом и акцентами `lighting` / `mood` / `composition` (совместимость с triple-variant в панели). **`finalPromptForGeneration`** = **`assembleVibeFinalPrompt(scene, willAttachReferenceInline)`**. Ответ: **`mergeOk: false`**, **`modelUsed: "passthrough"`**, **`llmProvider: "none"`**. Константы **`LEGACY_EXPAND_PROMPT_2C23CE94`** / merge в коде остаются только как справка / история.
...
- **Логи (extract/expand):** extract: `gemini_request` / `gemini_response` / `extract_parse_ok` и аналоги OpenAI. expand: **`[vibe.expand] scene_passthrough_ok`**. Общие: **`PIPELINE_FAIL`**, `extract_pipeline_failed` / `expand_failed` (unhandled). При `GEMINI_VIBE_DEBUG=1` — превью текста extract и для OpenAI (`landing/src/lib/gemini-vibe-debug-log.ts`).

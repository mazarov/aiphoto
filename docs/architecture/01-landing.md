...
- **Inline compose UI:** точный `promptText`, который уходит в generate, показывается foreground glass-карточкой; модели — квадратные config-driven tiles со стоимостью, формат/качество — компактные selects. После completion result image становится full-bleed backdrop со scrim, без отдельного foreground preview.

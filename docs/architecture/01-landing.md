...
- **Точка входа карточки `/p`:** treatment — `CardPageClient` → `onInternalGenerate`. Гость: `beginGuestCardRepeatAuth` + AuthModal, карточка остаётся. Authed: `GenerateDock.seedFromCard` + close overlay / `router.replace(last listing)` (не `history.back`). Control — LexyGPT.

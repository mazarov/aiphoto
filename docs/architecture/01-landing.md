...
> Последнее обновление: 2026-08-07 (**unified prompt edit chain:** до и после результата `CardInlineGeneratePanel` использует одну полноразмерную шторку с двумя блоками: большой редактируемый `Текущий промпт` и инструкция `Что изменить?`. `Применить изменение` только переписывает draft через Gemini; отдельный постоянный CTA `Сгенерировать` создаёт initial/child generation. Для child worker использует completed result parent как единственный image input. Миграция `171` добавляет owner-validated lineage, исходные `prompt_cards/prompt_variants` не изменяются.)
>
> Коррекция 2026-08-07 (**edit and generate are separate:** первый textarea можно менять вручную; второй `changeRequest` опционально переписывает первый, не enqueue-ит job и остаётся в той же шторке. После completion draft уже равен точному отправленному prompt последней generation, поэтому ручная правка и Gemini-remix продолжают цепочку от него. Генерация выполняется только отдельной кнопкой `Сгенерировать`.)
>
...
| `/api/prompt-remix` | POST (auth + internal generation allowlist): принимает текущий editable `prompt + changeRequest` и optional owned completed `parentGenerationId`, возвращает только переписанный prompt без создания generation. Модель `GEMINI_PROMPT_REMIX_MODEL` (default `gemini-2.5-flash`), `thinkingBudget=0` (rewrite не требует reasoning), `maxOutputTokens=8192`; ответ с `MAX_TOKENS` не принимается как готовый. Proxy определяется через `photo_app_config.gemini_use_proxy` + `GEMINI_PROXY_BASE_URL` |
...
- **Inline compose UI:** исходный `promptText` карточки инициализирует локальный draft. До и после result одна и та же iOS-style prompt-шторка показывает два блока: flex-height editable `Текущий промпт` и compact `Что изменить?`. `Применить изменение` вызывает text-remix и заменяет draft, но не закрывает шторку и не запускает image generation. Collapsed preview и enqueue используют текущий draft без мутации `prompt_cards`. Controls `Ваши фото` / `Модель` collapsed по умолчанию в square summary tiles; их sheets поддерживают photo multi-select/upload/delete и config-driven model/format/quality. После completion result image становится full-bleed backdrop, а footer показывает `Скачать` + отдельный `Сгенерировать`.
- **Inline engine:** prompt edit: `POST /api/prompt-remix { prompt: draft, changeRequest, parentGenerationId? }` → только новый draft; optional parent проверяется как owned completed, но текущий вручную редактируемый prompt остаётся source для rewrite. Initial generation: draft + выбранные `storagePath[]` → `POST /api/generate` (`vibeId=null`, `cardId`). Child generation: draft + `parentGenerationId` → `POST /api/generate`, worker использует parent result object. Enqueue сохраняет фактически отправленный текст в `landing_generations.prompt_text`; после enqueue клиент синхронизирует `draftPrompt/submittedPrompt`. Result menu переиспользует `GenerationCardMenu` без bulk-select: share/download/copy, `save-to-library`, `publish`, `DELETE`.

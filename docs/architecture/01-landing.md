...
|---------------|-----------|
| Dockhost / CI | **`docker build -f landing/Dockerfile .`** из корня клона `aiphoto/` (контекст — репозиторий, не только `landing/`). В builder кладётся `extension/sidepanel` рядом с `landing/` (`/work/extension/sidepanel`), иначе падает **`npm run build:stv-web`**. В образе нет корневого `package-lock.json` → трейсинг от каталога приложения → плоский `standalone/server.js`; runner копирует в `/app`. |
| Локально `next build` из `landing/` | Если в родителе есть `package-lock.json` → трейсинг от корня монорепо → `standalone/landing/server.js` (см. `next.config.ts`). Образ Docker при этом по-прежнему собирается командой из строки выше. |

### Правила сборки (чеклист)

1. **`npm run build` в `landing/`** — всегда включает **`build:stv-web`**; рядом в клоне должен быть каталог **`extension/sidepanel`** (см. `landing/scripts/build-stv-web.mjs`).
2. **Docker / Dockhost / CI** — **`docker build -f landing/Dockerfile .`** с **контекстом корня репозитория**; не задавать контекст только **`landing/`**.
3. Размер контекста — корневой **`.dockerignore`** в `aiphoto/`.
4. После правок образа — **`landing/scripts/verify-docker-image.sh IMAGE:TAG`**.
5. Постоянные правила для агента/ревью — **`.cursor/rules/landing-docker-next-standalone.mdc`**.

**Как не повторить**

1. **Контекст Docker** — корень репозитория; корневой **`.dockerignore`** ограничивает размер контекста. Не собирать образ из **`landing/`** одной папкой — не хватит `extension/sidepanel`.

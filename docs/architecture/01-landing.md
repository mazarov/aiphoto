...
/extension-stv          → Превью маркетингового лендинга расширения (спека `docs/extension-landing-pain-hope-solution.md`); публичный заголовок страницы — **Image To Prompt**; `metadata.robots` noindex; CTA: env **`NEXT_PUBLIC_STV_CHROME_STORE_URL`** (иначе заглушка `#stv-chrome-store`). Контент: hero + Pinterest-мок + pain + **Reference** (`PainReferenceVsDraftMock`) + **Q&A** (`ExtensionStvFaq`, `<details>`). Блок **Reference**: **Add photo** → `POST /api/upload-generation-photo` → signed URL → `POST /api/vibe/extract` → `POST /api/vibe/expand`; промпт в колонке draft = `finalPromptForGeneration` (тот же pipeline, что в `stv-core.js`); требуется авторизация (cookie).
```

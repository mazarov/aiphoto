...
**Каталог vs ИИ-фотосессия:** `promty-dlya-foto-*` (кроме хабов пар и девушки), `/stil/`, `/sobytiya/`, object L1 (`/s-mashinoy` и соседи) — один кадр: скопировать промт или Repeat. Кластер «промты для ИИ фотосессии …» принадлежит `/ii-fotosessiya/*`, **кроме** `/promty-dlya-foto-par` (пары) и explorer-H2 на `/promty-dlya-foto-devushki` («промт для фотосессии девушки», не «ИИ фотосессии женские»). `/ii-fotosessiya/zhenskie` остаётся владельцем «женская ИИ фотосессия». Чип «ИИ-фотосессия» в `popularLinks` на прочих L1 — сосед (серия луков), не замена Title/H1 хаба.
...
│   ├── listing-catalog-hub.ts  ← диспетчер хабов каталога (пары, девушки)

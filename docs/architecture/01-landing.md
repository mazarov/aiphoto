...
> Последнее обновление: 2026-08-07 (**inline image edit chain:** после результата `CardInlineGeneratePanel` заменяет повтор на CTA «Изменить картинку». Authenticated `POST /api/prompt-remix` в этом Landing переписывает prompt через `gemini-2.5-flash` и общие `GEMINI_API_KEY` / proxy-настройки; затем `POST /api/generate` создаёт новую job с `parentGenerationId`, а worker использует completed result parent как единственный image input. Миграция `171` добавляет owner-validated lineage, исходные `prompt_cards/prompt_variants` не изменяются.)
>
> Коррекция 2026-08-07 (**editable initial prompt:** полноэкранная шторка промпта до первой генерации содержит controlled Remix textarea. Клиент отправляет изменённый draft вместо исходного текста карточки и после enqueue фиксирует его как текущий prompt. Последующие image-edit итерации по-прежнему читают `prompt_text` последней completed parent generation, поэтому цепочка не возвращается к исходному `prompt_cards`.)
>
...
/pricing                → Preview для `azarov.maxim@gmail.com`; без авторизации доступен только по ссылке `/pricing?test=true`, иначе 404. Пакеты: Проба, Старт, Про, Максимум; до ЮKassa кнопка показывает non-blocking статус «Оплата появится скоро»
...
| `/api/prompt-remix` | POST (auth + internal generation allowlist): принимает owned completed `parentGenerationId + changeRequest`, сервер читает канонический текущий `landing_generations.prompt_text` и вызывает Gemini text. Модель `GEMINI_PROMPT_REMIX_MODEL` (default `gemini-2.5-flash`), proxy определяется через `photo_app_config.gemini_use_proxy` + `GEMINI_PROXY_BASE_URL` |
...
- **Inline photo library:** `CardInlineGeneratePanel` при открытии читает `GET /api/user-generation-photos`, показывает persistent-карусель квадратных preview по `created_at DESC` и автоматически выбирает самое свежее. Карусель остаётся доступна поверх готового result backdrop для повторной генерации. Новые файлы сразу проходят client prepare → `POST /api/upload-generation-photo` с `saveToLibrary=true`; выбор отмечается галочками, для одной генерации разрешено 1–10 фото. Удаление идёт через `DELETE /api/user-generation-photos/[id]`.
- **Inline compose UI:** до первого result исходный `promptText` карточки инициализирует локальный draft; полноэкранная prompt-шторка показывает Remix textarea, где текст можно изменить без мутации `prompt_cards`. Collapsed preview и `POST /api/generate` используют draft, а после enqueue foreground показывает точный отправленный prompt. Controls `Ваши фото` / `Модель` collapsed по умолчанию в два square summary tiles. Клик открывает один absolute iOS-style bottom sheet на всю ширину корневого compose-блока и поверх CTA; photo sheet поддерживает multi-select/upload/delete, model sheet — config-driven tiles со стоимостью и compact selects формата/качества. Sheet использует непрозрачную белую surface, а отдельный backdrop покрывает и затемняет весь compose. При enqueue sheet закрывается, progress/проценты переходят внутрь CTA. После completion result image становится full-bleed backdrop с лёгким scrim и прозрачными glass-поверхностями; footer перестраивается в пару `Скачать` + `Изменить картинку`. Последний CTA открывает sheet с текущим prompt и полем «Что изменить?».
- **Inline engine:** initial run: текущий prompt draft + выбранные `storagePath[]` → `POST /api/generate` (`vibeId=null`, `cardId`) → `generationId` → poll; enqueue сохраняет фактически отправленный текст в `landing_generations.prompt_text`. Edit run: клиент отправляет `parentGenerationId + changeRequest` в локальный `/api/prompt-remix`; сервер по owned generation читает последний `prompt_text`, Gemini строит следующий prompt, затем `POST /api/generate { parentGenerationId }` создаёт child job. API и enqueue RPC проверяют owned completed parent, worker читает его объект из `web-generation-results`. Каждая итерация становится новым parent; UGC card остаётся best-effort projection и не используется как source of truth. Result menu переиспользует `GenerationCardMenu` без bulk-select: share/download/copy, `save-to-library`, `publish`, `DELETE`.

...
- **`getCardPageData`:** в ответе для клиента — `photoMeta[]` (bucket/path/url, параллельно `photoUrls`) для debug-действий. В жёлтой DEBUG-панели на `CardPageClient` (при включённом debug-режиме) — кнопка **«Сделать „Было“»**: текущий слайд карусели → `POST /api/set-before` (`prompt_card_before_media`), локально обновляются превью «Было» и список фото без перезагрузки; **«Удалить карточку»** → `POST /api/debug-delete-card` (подтверждение в браузере + `confirmSlug`), редирект на `/`.
...
- **Активация:** 5 быстрых кликов по логотипу в **header** только на **`/`** (`useDebugLogoToggle` → `DebugProvider.toggleDebug`); состояние в `localStorage` (`debug_open`). Одиночный клик на главной — scroll to top (как раньше).
- **Debug-хаб:** `HomeDebugCatalog` на `/` — `FilterableGrid` с `enableDebugPanel={true}` и панелью фильтров (ID, warnings, score, RU prompt, тег, «было», **датасет**).
- **Категорийные листинги** (`/[...slug]`): при `debugOpen` — только overlay на карточках (`PromptCard` / `GroupedCard`); панель фильтров **не** показывается (`enableDebugPanel={false}`).
- **Фильтр по датасету:** глобальный запрос `/api/search-cards?dataset=…` с пагинацией (48 за страницу, «Загрузить ещё»); все карточки источника в одном grid на главной. Выбор датасета не на `/` → redirect `/#debug-catalog` + `pendingDataset` в контексте.
- **`/p/[slug]`:** жёлтая debug-панель на `CardPageClient` (удаление, «Сделать было») — без изменений.
...
| CardFilters | `components/CardFilters.tsx` | `FilterableGrid`: debug overlays; панель фильтров только при `enableDebugPanel` (главная) |
| HomeDebugCatalog | `components/HomeDebugCatalog.tsx` | Debug-хаб на `/`: grid + фильтры по всей базе |
| useDebugLogoToggle | `hooks/useDebugLogoToggle.ts` | Жест 5× клик по header logo на главной |
...
| DebugFAB | `components/DebugFAB.tsx` | `DebugProvider`: `debugOpen`, `panelOpen`, `pendingDataset`; localStorage |

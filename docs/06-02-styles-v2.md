...
**Callback примера:** `style_example_v2:{substyle_id}:{group_id}`
**Callback назад:** `style_groups_back:{group_id}`
...
// Назад к группам (с сохранением group_id)
...
// Запускаем генерацию с введённым промптом
```
...
- [x] Хендлер `style_v2:*`
- [x] Хендлер `style_groups_back` (с editMessageText)
- [x] Хендлер `style_example_v2:*` (примеры подстилей)
...
- [x] Изменить change_style handlers
- [x] Изменить back_to_styles handler

### Фаза 3: Тест
- [x] Тест на своём аккаунте (в процессе)
- [ ] Проверить все группы
- [ ] Проверить генерацию для каждого подстиля
- [x] Проверить кнопку "Назад"
- [ ] Проверить "Свой стиль" (custom)
- [ ] Проверить кнопки "Пример"
